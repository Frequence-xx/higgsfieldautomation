...
**⚠️ FUTURE WATCH — Seedream 5.0 Pro (launched July 8, 2026 — NOT yet on AIMLAPI as of 2026-07-10):** ByteDance released this model as "Seedream 5.0 **Pro**" (not "Full" as was anticipated). **Actual specs differ from earlier predictions:** max **10 reference images** (not 14), **2K max output** (not 4K), BytePlus model IDs `doubao-seedream-5-0-pro-260628` (Volcano Engine/China) and `dola-seedream-5-0-pro-260628` (BytePlus international). New editing modes over Lite: region editing, sketch editing, anchor editing, **layer separation** (PNG transparency output), multi-image fusion. Quality tiers: `"Basic"` (1K) / `"High"` (2K). 9:16 via `aspect_ratio: "9:16"`. Currently available on fal.ai, Runware, Wavespeed, BytePlus — NOT on AIMLAPI (not actionable under AIMLAPI-only policy). Expected AIMLAPI string when it lands: `bytedance/seedream-5-0-pro`. If it lands on AIMLAPI: at 10 refs it becomes a strong cheaper NBP Edit fallback ($0.075–$0.15/img vs NBP's $0.195), with the layer separation capability adding post-production value for background extraction. **CANARY REQUIRED** before production use once AIMLAPI confirms.
