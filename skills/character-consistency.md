...
**LaVieID — Future Watch (arXiv 2508.07603, ACM MM 2025, code + weights released; pass 40 finding, 2026-08-17; SC303 update, 2026-08-29):** Local Autoregressive Diffusion Transformers for Identity-Preserving Video Creation. Code: github.com/ssugarwh/LaVieID. **Code IS released** (4 commits). **Pretrained checkpoint now downloadable from ModelScope** (SC303 finding, 2026-08-29 — previously code-only). Inference script: `infer_facevideo_router_v2.sh`. Tested environment: Python 3.12.4, PyTorch 2.1.0+, CUDA 12.8 (Python 3.10 recommended if xformers/diffusers compatibility issues arise). No VRAM requirements documented — check ModelScope checkpoint size before attempting local run. Key mechanisms: (1) **Local Router** — represents latent states as weighted combinations of fine-grained local facial structures, directly alleviating feature interference between identity cues and background/clothing content (the mechanism that causes our known identity drift under large head rotations); (2) **Temporal Autoregressive Module** — divides denoised latent tokens into temporal chunks and exploits long-range temporal dependencies to predict correction biases per chunk before video decoding, significantly improving inter-frame identity consistency over 5s clips. No AIMLAPI endpoint as of 2026-08-29. **Practical implication now:** The Local Router validates our 4-ref strategy — providing geometrically distinct face views gives the model distinct "local structures" to anchor against, reducing feature interference. The temporal chunk correction mechanism suggests that longer clips (10s+ at O3) benefit more from this architecture than short 5s clips; for our 5s standard, Kling O1 elements remain the correct choice until a LaVieID-based endpoint appears on AIMLAPI. **If local testing is feasible** (i.e., GPU available): download checkpoint from ModelScope and test Karel/Mourad identity lock — ModelScope checkpoint availability makes this the first open-weights model with a validated ACM MM paper for identity-preserving video that can now be run locally.
...
- **Angular diversity > expression diversity:** front + 3/4 + profile is the minimal effective multi-view set (confirmed by Mv²ID and MoFE research). Do NOT replace an angle ref with an expression variant — cover angles first. **MV-S2V (arXiv 2601.17756, SIGGRAPH 2026) validates this:** introduces Temporally Shifted RoPE (TS-RoPE) to architecturally distinguish between different subjects AND distinct views of the same subject in reference conditioning — proving that distinct viewpoints carry unique identity signals that need explicit disambiguation, not redundancy. No production code; research validation only.
...
resp = httpx.post("https://api.aimlapi.com/v2/video/generations", json={
    ...
    # ELEMENT REFERENCE SYNTAX (SC149+SC177+SC296 correction):
    # Correct syntax: @element_name in prompt (name-based, matching "name" field in kling_elements)
    # <<<element_1>>> is for persistent library/saved assets ONLY — NOT for inline kling_elements
    # @image_1 through @image_7: raw image references WITHOUT element binding (SC296 finding)
    # AIMLAPI canary: use @element_name — confirmed correct on O3; <<<X>>> is different use case
...
- Prompt references elements: **`@element_name`** (name-based, matching the `"name"` field in `kling_elements`) — confirmed correct syntax on O3 official API and most wrappers (kie.ai: `@element_dog` → `{"name": "element_dog", ...}`). **IMPORTANT (SC296 clarification):** `<<<element_1>>>` is for **persistent library/saved assets only** (a separate workflow where elements are pre-registered in a library) — it is NOT a fallback for inline `kling_elements`. Do NOT use `<<<element_1>>>` for inline element binding. `@image_1` through `@image_7` are for raw image references without element binding. When O3 lands on AIMLAPI, use `@element_name` and do NOT fall back to `<<<element_1>>>` for inline elements. (pass 22 correction: confirmed @element_name is primary; SC296: clarified <<<X>>> is different use case entirely)
...
**WildActor — Future Watch (arXiv 2603.00586, March 2026, ICML 2026 accepted; code updated June 2026):** Full-body identity consistency across unconstrained viewpoints and motions. Code: github.com/WildActor/WildActor. Dataset: Actor-18M (1.6M videos, 18M human images). Benchmark: Actor-Bench.

**SC303 update (2026-08-29):** Accepted to **ICML 2026**. In June 2026, the team released **Wan2.2-5B-compatible inference code** and the Actor-18M construction pipeline. Model weights are **still unreleased** (listed as TODO in the repo). No AIMLAPI endpoint. Backbone upgraded from original to Wan2.2-5B — when weights drop, local testing becomes feasible on 16GB+ GPU (Wan2.2-5B is leaner than 14B variants).
...
3. **No AIMLAPI endpoint** as of 2026-08-29. Monitor for hosted API — Wan2.2-5B backbone makes AIMLAPI adoption more plausible than the original custom-backbone version.
...
**HarmoView — Future Watch (arXiv 2606.10839, June 22, 2026, no public code as of 2026-08-25; pass 44 finding):** Harmonizing Multi-View Constraints for Identity-Consistent Video Generation. Evaluated on 100 manually-curated cases, 52 unique identities — matches leading closed-source engines. Three key mechanisms: (1) **Multi-level Feature Injection (MFI)** — injects raw ViT features from the frontal reference alongside text tokens via cross-attention, anchoring identity fidelity from the face's most informative view; (2) **Jump-RoPE** — identity-wise feature isolation to reduce identity crosstalk between different views during generation (addresses the copy-paste artifact risk our 4-ref cap aims to limit); (3) **Progressive View Curriculum** — 4-stage training strategy using view dropout for stable transition from vanilla T2V to high-fidelity spatial reasoning. No public code or AIMLAPI endpoint. **Practical implication:** MFI validates `front.png` as the primary identity anchor — it carries the richest ViT features. Jump-RoPE validates the 4-ref cap: more views = more crosstalk unless architecturally decoupled; our cap limits that risk. When view count matters, prioritize frontal quality over adding extra angle refs.
