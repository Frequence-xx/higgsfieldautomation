...
| Seedream 4.5 Edit | `bytedance/seedream-4-5`✧ | Reference-based portrait/character editing; alternative to NBP Edit when blocked by OTHER — ByteDance, strong face fidelity | 3 | ~$0.08 (est.) | 9:16 (canary) |
| Seedream 5.0 Lite | `bytedance/seedream-5-0-lite-preview`✧ | T2I draft tier; brand color stills — cheaper than NBP Pro | 0 | ~$0.04 (est.) | 9:16 (canary) |
...
✧**Seedream 4.5 and 5.0 Lite Preview (2026-06-06):** ByteDance Seedream models confirmed on AIMLAPI. Seedream 4.5 Edit supports reference-based image-to-image (up to 3 refs) with strong face fidelity — potential fallback when NBP Edit fires `blockReason: OTHER` on character composites. Seedream 5.0 Lite Preview is T2I only (no refs). **AIMLAPI model strings, exact pricing, 9:16 support, and parameter names are UNVERIFIED** — run canary before any production use. Do NOT use as primary character model until canary confirms face adherence vs. our NBP Edit baseline. If canary passes: Seedream 4.5 Edit becomes a lower-cost fallback ($0.08 est. vs NBP Edit $0.20) for shots blocked by March 2026 OTHER policy. AIMLAPI docs: `https://docs.aimlapi.com/api-references/image-models/bytedance/`.
...
NBP Edit returns blockReason OTHER (March 2026 policy)? → (1) Use stylized ref + add "fictional, illustrated character" to prompt (60-70% block reduction); (2) if still blocked: Qwen Image Edit ($0.059) or Seedream 4.5 Edit (canary first, $0.08 est.) for surgical edit; (3) T2I describe-the-scene fallback
```
...
- `guidance_scale` range on AIMLAPI: 1–20. **Default is 3.5.** Two regimes: (1) character/face editing → use 2.0–2.5 (more image-preserving, prevents face warp; 2.0 is the confirmed lower bound for stable face output); (2) text/typography editing → use 3.5–4.0 (more prompt-literal for letter accuracy). Do not exceed 5 for character editing — face structure distorts. Note: other platforms (Replicate, fal.ai) use a different guidance_scale range (1–50) — do not import their settings. **Kontext technical paper (BFL, June 2026): arxiv.org/abs/2506.15742 — confirms guidance_scale=2.5 is the recommended default for image-editing operations in the official Hugging Face diffusers integration. AIMLAPI's guidance_scale range (1–20) maps to diffusers' full range — use 2.0–2.5 on AIMLAPI for character editing as already documented.**
- **`image_strength`** (0–1, default 0.1): Controls how much the reference image influences the output. **Status: CONFIRMED on AIMLAPI Kontext endpoint (2026-05-31).** Increase to 0.3–0.5 for stronger face identity lock (higher = more reference-faithful, less prompt flexibility). Use 0.3–0.5 for character editing; keep at default 0.1 for background/environment changes where you want prompt to dominate. **Scale clarification (2026-06-06):** AIMLAPI's `image_strength` (higher = more reference) is the INVERSE of HF diffusers' `strength` parameter (lower = more reference-preserving). Do NOT import guidance_scale or strength values from diffusers tutorials to AIMLAPI calls — the numeric direction is opposite. AIMLAPI: 0.5 = strong identity lock. Diffusers: 0.5 = allow 50% repaint (moderate identity loss).
