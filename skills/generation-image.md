...
| Grok Imagine Image 2.0 | NO API YET† (**launched 2026-08-07 — API "coming soon"**) | #2 Arena T2I + editing; up to 5 refs (up from 3 on Quality); region editing, designer typography; 9 aspect ratios via Smart Resize; replaces Grok Imagine Quality model | 5 | $0.05/img est | 9:16 supported |
...
**⚠️ 2026-08-07: Grok Imagine Image 2.0 launched.** xAI replaced the Quality model with Image 2.0 on August 7, 2026. Key upgrade: up to **5 reference images** (vs 3 on Quality), region-specific editing tools, auto-resize across 9 aspect ratios, designer-grade typography. Arena ranking: **#2 for both T2I and image-editing** (behind GPT Image 2). Pricing: $0.05/img at 1K (same as Quality tier). **API status: "coming soon"** per launch coverage — the model is consumer-only (grok.com, iOS, Android) at launch; developer API not yet open as of 2026-08-15. Monitor AIMLAPI additions. When it does land on AIMLAPI, expected model string: `x-ai/grok-imagine-image-2.0`. At 5-ref ceiling (vs Quality's 3) and #2 Arena ranking, this becomes a stronger cheap B-roll / environment draft alternative than the Quality model was — but still not suitable for character shots (no face adherence). Do NOT use before canary confirms API availability on AIMLAPI. **2026-08-16 recheck: API still consumer-only (grok.com, iOS, Android) — no developer API announced.**
...
⊘**Qwen-Image-3.0 (launched 2026-07-21, GA 2026-08-05 — NO API ON AIMLAPI as of 2026-08-15):** Alibaba's third-generation image model. Available in two editions: **Qwen-Image-3.0-Pro** (native Alibaba API model ID: `qwen-image-3.0-pro`) and **Qwen-Image-3.0-Standard** (model ID: `qwen-image-3.0-standard`). Both went fully open on Alibaba Cloud Bailian/DashScope on August 5, 2026 (from invite-only preview since July 21). Key specs: up to 4.5K token prompt input; sub-10px text legibility in 12 languages (Dutch confirmed); complex multi-panel layout generation (newspapers, storyboards, menus, exam papers in one pass); image editing that preserves character appearance across scenes; 9 images at once in one call; micro-expression and pore-level fidelity in portraits. **Pipeline status: NOT actionable under AIMLAPI-only policy — still NOT on AIMLAPI as of 2026-08-15.** If AIMLAPI adds it, expected model strings: `alibaba/qwen-image-3-0-pro` or `alibaba/qwen-image-3-0` (unconfirmed). Primary use case if added: CTA cards requiring Dutch text at sub-10px legibility, and character compositing with 4.5K token multi-ref prompts. **Monitor AIMLAPI model releases — especially relevant given Qwen-Image-Edit-2509 is already on AIMLAPI.**
...
Need cheap B-roll scenery draft (no characters)? → Wan 2.7 Image Pro ($0.06, `alibaba/wan-2-7-image-pro`, 4K T2I, Dutch text support) — CANARY REQUIRED; Grok Imagine Image 2.0 (API "coming soon" as of 2026-08-15, 5 refs, $0.05/img — monitor) replaces the deprecated Quality model when it lands on AIMLAPI
