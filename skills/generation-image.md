...
**Note (2026-08-24 pass 43 recheck):** MAI-Image-2.6 (#2 Arena T2I, +79 Elo over 2.5) still NOT on AIMLAPI as of 2026-08-24 — AIMLAPI docs/product search returns only MAI-Image-2.5; no 2.6 page found. Available in MAI Playground, Microsoft Foundry, and Arena Direct Mode. Continue monitoring. Once confirmed, evaluate as upgrade path over MAI-Image-2.5 for brand stills — especially for Dutch text-heavy CTA cards (text rendering improved +91 Elo). See ⟁² footnote above.

⟁²**MAI-Image-2.6 (launched 2026-08-10 — NOT on AIMLAPI as of 2026-08-24 pass 43 recheck):** Microsoft's newest image generation model. Arena T2I ranking: **#2 (Elo 1336)** — beats NB2 (NB Pro), Meta Muse Image, and Grok Imagine Image 2.0; only GPT Image 2 ranks higher. Key improvements over MAI-Image-2.5: **+79 Elo overall**, **+91 Elo on text rendering specifically**, better portrait fidelity, 3D renders, and commercial product imagery. Reference image (I2I) support confirmed; same family as 2.5. **API rollout timeline:** consumer Arena access first; MAI Playground + Microsoft Foundry launched week of Aug 17, 2026. Third-party API providers (OpenRouter, etc.) expected shortly after Foundry launch. **NOT on AIMLAPI as of 2026-08-24 (pass 43 recheck) — AIMLAPI docs/product search shows no 2.6 page; monitor AIMLAPI for `microsoft/mai-image-2.6` addition.** When it lands: primary use case = highest-quality T2I/I2I for brand stills and CTA cards (expected to improve over MAI-Image-2.5 which already scores #2 Arena editing). Pricing: TBD (expected similar to or slightly above 2.5; canary required). 9:16 aspect ratio supported (same family convention as 2.5). **CANARY REQUIRED** before any production call once AIMLAPI adds it.
...
**Flux Kontext Max parameters — guidance_scale and steps (2026-08-18):** fal.ai docs (proxy for BFL native params) confirm:
- `guidance_scale` default: **3.5** — controls prompt adherence vs. creative freedom. Range 1–20. For character identity lock (must preserve face exactly), try **4.0–5.0**; higher values increase fidelity to the reference but reduce layout creativity. Do NOT exceed 7 — produces over-sharpened artifacts at high values.
