...
| MAI-Image 2.5 Flash | `microsoft/mai-image-2.5-flash`⟁ (**NOT confirmed on AIMLAPI as of 2026-07-13 — monitor**) | Faster/cheaper variant of MAI-Image 2.5; same T2I + I2I; ~30% cheaper than full on output tokens (~$33/M vs $47/M); available on Azure AI Foundry + OpenRouter | TBD | token-based: $1.75/M text+image input / ~$33/M image output (~$0.03–0.10 est per img) | 9:16 supported |
...
**AIMLAPI model string status (2026-06-20):** xAI native string is `grok-imagine-image-quality` (confirmed). AIMLAPI docs index still shows `x-ai/grok-imagine-image` and deprecated `x-ai/grok-imagine-image-pro` — Quality model page NOT visible in AIMLAPI docs as of this research pass. Path-style strings (`xai/grok-imagine-image-quality/text-to-image`, `/edit`) from prior research are third-party only and not confirmed on AIMLAPI. Expected AIMLAPI string following their `x-ai/` convention: **`x-ai/grok-imagine-image-quality`** — **CANARY REQUIRED** before production use. Prompt syntax and parameters identical to Pro — no prompt changes needed. Do NOT use for character shots (3-ref ceiling, no face adherence).
...
**For our pipeline:** If AIMLAPI adds it, Gemini Omni Flash could replace Kling I2V for B-roll and scene edits at comparable cost ($0.10/sec vs Kling v3 Standard $0.218/sec — cheaper, but 10-second max vs Kling's 5-second clip). Use case: iterative scene edits where Kling would require full regeneration. **AIMLAPI status: NOT CONFIRMED** — available on Google AI Studio / Gemini API directly (`gemini-omni-flash-preview`), confirmed on kie.ai, but NOT in AIMLAPI model database as of 2026-07-10. Do NOT route production calls until AIMLAPI confirms. Full documentation in `skills/generation-video.md` (update needed).
