...
| MAI-Image 2.5 Flash | `microsoft/mai-image-2.5-flash`⟁ (**NOT confirmed on AIMLAPI as of 2026-07-21 — monitor**) | Faster/cheaper variant of MAI-Image 2.5; same T2I + I2I; ~59% cheaper than full on image output (~$19.50/M vs $47/M confirmed); available on Azure AI Foundry + OpenRouter | TBD | token-based: $1.75/M text+image input / **$19.50/M image output** (~$0.02–0.07 est per img) | 9:16 supported |
...
| Qwen-Image-3.0 | NO API ON AIMLAPI YET⊘ (**released 2026-07-21 — Alibaba-only**) | T2I + I2I; 4.5K token input; sub-10px text legibility; 12 languages incl. Dutch; complex layout generation; image editing (maintains character appearance across situations); agent-style grounding. Available only on Qwen Chat, Qwen Studio, and Alibaba API. No benchmarks or weights published at launch. **Monitor for AIMLAPI addition.** | TBD | TBD | TBD |
...
⊘**Qwen-Image-3.0 (released 2026-07-21 — NO API ON AIMLAPI as of 2026-07-26):** Alibaba's third-generation image model. Available ONLY through Qwen Chat, Qwen Studio (web/iOS/Android/macOS/Windows), and Alibaba's own API — NOT via AIMLAPI. Key specs: up to 4.5K token prompt input (vs Qwen-Image 2.0's shorter context); sub-10px text legibility in 12 languages (Dutch confirmed); complex multi-panel layout generation; image editing that preserves character appearance across scenes; 9 images at once in one call. No benchmarks published and no public weights released at launch. Launch controversy: Alibaba pitched it against GPT Image 2 across 19 tests but the community noted the comparison methodology was unpublished. **Pipeline status: NOT actionable under AIMLAPI-only policy.** If AIMLAPI adds it, expected model string: `alibaba/qwen-image-3-0` (unconfirmed). Primary use case if added: CTA cards requiring Dutch text at sub-10px legibility, and character compositing with 4.5K token multi-ref prompts. **Monitor AIMLAPI model releases for addition — especially relevant given Qwen-Image-Edit-2509 is already on AIMLAPI.**
...
Need cheap B-roll scenery draft (no characters)? → Grok Imagine Quality ($0.055, `x-ai/grok-imagine-image-quality`) — CANARY REQUIRED (Pro deprecated May 15; Quality string unverified on AIMLAPI); OR Wan 2.7 Image Pro ($0.06, `alibaba/wan-2-7-image-pro`, 4K T2I, Dutch text support) — CANARY REQUIRED
