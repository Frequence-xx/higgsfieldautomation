...
**Confirmed AIMLAPI Kling model roster (July 5, 2026):** Kling 2.6 Pro, Kling v3 Standard I2V, Kling v3 Standard T2V, Kling v3 Pro I2V, Kling v3 Pro T2V, **Kling v3 Standard Turbo I2V, Kling v3 Standard Turbo T2V, Kling v3 Turbo Pro I2V, Kling v3 Turbo Pro T2V** (all launched June 17, 2026 — canary required), Kling O1 Reference-to-Video, Kling O1 Video-to-Video Reference, Kling O1 Video-to-Video Edit, Kling 2.6 Pro Motion Control. **Also on AIMLAPI (NOT in routing matrix):** Kling v2 Master I2V/T2V and Kling v2.1 Master I2V/T2V (SC258 Aug 15, 2026 — found as dedicated docs.aimlapi.com pages; pricing ~$1.70/5s; more expensive than v3 Pro at $1.46 and older architecture — do NOT use for Snelverhuizen; v3 Pro is cheaper + higher resolution ceiling). **⚠️ RETIREMENT NOTICE (SC300 Aug 28, 2026):** Kling 1.0, 1.5, 1.6, 2.0 Master, 2.1, and 2.1 Master are confirmed retiring September 15, 2026 on the native Kling API — the v2 Master and v2.1 Master models listed here will be removed from AIMLAPI around the same date. These were already marked do-not-use; no pipeline impact since we use v3 Standard/Pro only. Kling 2.6 Pro is NOT in the retirement list — safe to use. **🚨 RETIREMENT IN 11 DAYS (as of Sept 4, 2026 — SC325 pass 43):** If any script still references Kling v2 Master or v2.1 Master strings, audit and remove now before they return 404 on Sept 15. No action needed for our v3 Standard/Pro pipeline. ⚠️ PRE-REMOVAL CONFIRMED: Kling v2 was already removed from Kling Image Generation (Kling's own native image-gen pipeline) ~Aug 28, 2026 (ComfyUI changelog — SC318), ahead of the full Sept 15 API retirement. This confirms the retirement is proceeding and Kuaishou is removing v2 in stages. AIMLAPI-side: v2-master and v1.x docs pages still visible in AIMLAPI docs search as of Sept 2, 2026 (not yet removed). **NOT on AIMLAPI (confirmed absent Sept 2, 2026 — SC318 pass 42 recheck; GitHub api-docs audit Aug 31-Sept 1 shows zero Kling commits):** Kling O3/Omni (`klingai/video-v3-omni` base string — see dedicated O3 section below for full model strings confirmed in AIMLAPI model database SC279; no new O3 docs page added Aug-Sept 2026; site:docs.aimlapi.com search confirms no O3 page as of Sept 2), Kling v3 Motion Control Standard/Pro (only v2.6 confirmed on AIMLAPI; v3 Motion Control is live on WaveSpeedAI, Eachlabs, Replicate, MindStudio — but not AIMLAPI as of Sept 2, 2026 (SC318 pass 42 recheck)), Kling 4K (`klingai/video-o3-4k` — officially announced June 12, 2026; Runware-confirmed; AIMLAPI TBD since O3 itself lacks a dedicated docs page), **Kling 4.0** (announced Q3 2026 — "Lite, Fast, Standard, Pro & 4K tiers"; longer clips beyond 15s, 4K native, multi-shot sequencing, more than 6 shots per generation; still "coming soon" per multiple industry sources as of Sept 2, 2026 — NOT on any provider API yet (SC318 pass 42); if you see a "Kling 4.0" generator it's running an earlier model; canary watch priority HIGH when it appears). **AIMLAPI Kling audit Sept 2, 2026 (SC318 pass 42):** Zero Kling-specific commits in AIMLAPI api-docs repo Aug 30-Sept 1, 2026 (GitHub commit audit). All Kling model strings, pricing, and parameters confirmed unchanged. Non-Kling additions in same window: anthropic/claude-fable-5-1 added Sept 1; minimax/h3-max added Aug 31; arcee-ai/virtuoso-large and allenai/olmo-3-32b-think hidden. Previously noted: Tencent HY4-Preview (LLM), Google Gemini Omni 1.1 Flash, Wan 3.0 Video.

**fal.ai naming note:** fal.ai renamed v3 endpoints to `o3` on April 10, 2026, then reversed back to `v3` on May 23, 2026. For v3 Pro on fal.ai, use `fal-ai/kling-video/v3/pro/image-to-video`. For O3/Omni on fal.ai, it's a separate model: `fal-ai/kling-video/o3/standard/image-to-video`. **Kling O3 is NOT on AIMLAPI as of September 2, 2026** — confirmed absent from AIMLAPI docs index (SC318 pass 42 recheck; no dedicated O3 page in site:docs.aimlapi.com Kling search results). The June 17, 2026 Turbo launch did NOT include O3/Omni on AIMLAPI — only the Turbo tier (Standard Turbo + Turbo Pro) was added. AIMLAPI's own blog post listing Kling models only mentions Kling 2.6 Pro and Kling v3 Pro. Also note: fal.ai Kling v3 Standard pricing ($0.084/sec) is ~2.6× cheaper than AIMLAPI ($0.218/sec) — the AIMLAPI-only mandate is a real cost penalty for Kling Standard tier.
