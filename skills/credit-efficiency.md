...
| Wan 2.7 R2V | `alibaba/wan-2-7-r2v` | 720p/1080p | ~$0.50 est. | **UNVERIFIED — Blog claims live, no dedicated docs page (SC213, 2026-07-15).** AIMLAPI blog explicitly states "all four modes including R2V via AIMLAPI" but no dedicated R2V docs page found on docs.aimlapi.com as of July 15. Upgraded from SC206's "NOT CALLABLE" — blog claim is stronger evidence than model database entry alone. **Run cautious canary before next production session:** call with `image_urls: [hero_frame_url]`, `aspect_ratio: "9:16"`, `duration: 5`. If valid job ID returned → R2V is live. Wan 2.6 R2V (`alibaba/wan-2-6-r2v`) remains confirmed fallback. Parameters: `image_urls` (max 3 image-only; ≤ 5 total mixed), `aspect_ratio: "9:16"`, duration 2-10s. |
