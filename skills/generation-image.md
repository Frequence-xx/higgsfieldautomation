...
| Meta Muse Image | NO API YET⚑ | T2I + I2I multi-ref compositing; #2 Arena for T2I + single-image + multi-image editing; agent-style: plans layout, pulls real-time web context, blends refs; NO public API as of 2026-07-21 — consumer-only (Meta AI, Instagram, WhatsApp). **Monitor for AIMLAPI addition.** | TBD | TBD | TBD |
...
⚑**Meta Muse Image (released 2026-07-07 — NO PUBLIC API as of 2026-07-21):** Meta Superintelligence Labs' first image generation model. Launched July 7, 2026 inside Meta AI, Instagram, and WhatsApp — consumer-only at launch. Key specs: agent-style generation (plans layout, pulls real-time web context via tool calls, refines output iteratively), multi-reference compositing, high-fidelity instruction following. **Arena rankings at launch (human preference Elo):** #2 for T2I, #2 for single-image editing, #2 for multi-image editing — ahead of NB2, behind only GPT Image 2 for text-heavy work. **API status (2026-07-21):** NO public API for Muse Image. Meta's Model API (launched July 9, 2026) gives developer access to **Muse Spark 1.1** (the text/reasoning model, $1.25/M input, $4.25/M output) — NOT Muse Image. Muse Image is confirmed NOT available via third-party APIs (no AIMLAPI, no Replicate, no Together AI listing). Monitor for API release. **AIMLAPI expected string when available:** `meta/muse-image` (unconfirmed). NOT actionable under AIMLAPI-only policy until Meta opens an image API. If the policy relaxes: primary use case would be multi-ref brand compositing and face-consistent character editing, given its #2 Arena multi-image editing score.
