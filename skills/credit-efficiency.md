...
|-----------|------------|------------|-----------|-------|
| Character close-up | NBP Edit ($0.195) | Kling v3 Pro I2V ($0.728) | **~$0.92** | Subject Binding 80-90, I2V from hero frame |
| Character — draft/iteration | NBP Edit ($0.195) | Kling v3 Standard I2V ($0.546) | **~$0.74** | Use Standard until prompt is dialed in, Pro for final only |
...
| Truck/product hero | NBP Edit ($0.195) | Kling v3 Pro I2V ($0.728) | **~$0.92** | cfg_scale 0.7, ghost-driving locks |
...
| Money shot / CTA | Flux Pro v1.1 Ultra ($0.10) | Kling v3 Pro I2V ($0.728) | **~$0.83** | Highest quality |
...
1. **Draft iterations:** Kling v3 Standard I2V (`klingai/video-v3-standard-image-to-video`) at $0.546/5sec — **use 3s clips for drafts (see below)**
2. **Final output:** Kling v3 Pro I2V (`klingai/video-v3-pro-image-to-video`) at $0.728/5sec — ONLY after prompt is approved by owner
3. **Savings (5s drafts):** Two Standard drafts + one Pro final = $1.82 vs three Pro passes = $2.18 — saves $0.36/clip (17%)
4. **Savings (3s drafts):** Two 3s Standard drafts + one Pro final = $1.39 vs three Pro passes = $2.18 — saves $0.79/clip (36%)
...
- Standard 3s clip cost: $0.1092/sec × 3s = **$0.33** (vs $0.546 at 5s — saves $0.22/draft)
...
|------|-------|----------|------|--------|
| Draft 1 | Standard | **3s** | $0.33 | Evaluate motion direction and identity lock |
| Draft 2 (if needed) | Standard | **3s** | $0.33 | Confirm prompt fix worked |
| Final | Pro | **5s** | $0.728 | Owner delivery |
...
**⚠️ KLING SEPT 15 RETIREMENT SCOPE (SC343 2026-09-09):** Only OLD models are retiring Sept 15: Kling v1.5, v1.6, v2.0 Master, v2.1, v2.1 Master. **Kling v3 Standard and v3 Pro (= Kling 3.0) are NOT being retired** — they are the current flagship. Pipeline continues using `klingai/video-v3-*` strings unchanged. No migration needed.
...
| Kling v3 Turbo Pro I2V (canary) | `klingai/video-v3-turbo-pro-image-to-video` | **1080p (9:16)** | **$0.91** ($0.182/sec — AIMLAPI pricing page confirmed June 2026) | **$0.546** (3s) — ⚠️ ROUTING VERDICT (SC360 Sept 14, 2026): v3 Pro now confirmed $0.728/5s — Turbo Pro at $0.91 IS pricier than regular Pro for same 1080p. **DO NOT route finals to Turbo Pro** — use v3 Pro I2V ($0.728). Only valid use case: A→B transitions using first=last frame ($0.91 for explicit endpoint control, when that feature matters). ✓ SILENT VIDEO by default in single-clip mode. |
...
| Kling v3 Pro I2V | `klingai/video-v3-pro-image-to-video` | 1080p (9:16) | **✅ SEARCH-CONFIRMED: $0.728/5s ($0.1456/sec — SC360 2026-09-14, AIMLAPI pricing page snippet "$0.1456 per second (variable)").** Old AIMLAPI price was $1.46/5s ($0.291/sec) — was 2.6× markup, now 1.3× ($0.112/sec native × 1.3 = $0.1456/sec). Audio behavior canary still needed; full invoice canary when unblocked. $15 session ceiling now covers ~20 Pro final clips. | $0.44 (3s at $0.1456/sec) |
| Kling v3 Pro T2V | `klingai/video-v3-pro-text-to-video` | 1080p (9:16) | **✅ SEARCH-CONFIRMED: $0.728/5s ($0.1456/sec — SC360 2026-09-14).** Same as I2V. | $0.44 (3s) |
...
| Wan 2.6 I2V Flash (**CONFIRMED ON AIMLAPI** — canary pricing) | `alibaba/wan2.6-i2v-flash` | 720p/1080p | **~$0.033/sec est.** (~$0.165/5s silent 720p) | ✓ CONFIRMED ON AIMLAPI: dedicated model page found at `aimlapi.com/models/alibaba-wan2-6-i2v-flash` (SC311 2026-08-31). Alibaba native: $0.025/sec silent 720p, $0.0375/sec silent 1080p; with AIMLAPI ~1.3× markup → ~$0.033/sec 720p (~$0.165/5s) est. Third-party comparison sites (NetMind, PiAPI) show $0.08/sec 720p — may reflect standard I2V not Flash; AIMLAPI billing canary urgently needed to confirm actual rate. If $0.033/sec confirmed: **beats Hailuo 2.3 Fast ($0.208/5s) by 21%**. I2V only. Audio param: try `audio_mode: "mute"` (Wan convention). CANARY PRIORITY: HIGH. |
...
| LTXV 2 Fast I2V | `ltxv/ltxv-2-fast` | 1080p | **$0.052/sec (was CONFIRMED)** | ❌ **BROKEN as of 2026-08-15.** Lightricks removed LTX-2 strings on Aug 15 — `ltxv/ltxv-2-fast` ERRORS on AIMLAPI. AIMLAPI never added replacement string as of SC262 (2026-08-16). Expected new string: `ltxv/ltxv-2-3-fast` (CANARY REQUIRED when/if AIMLAPI adds it). Use Hailuo 2.3 Fast ($0.0416/sec) for all non-char I2V. Watch: **LTX-2.5** released 2026-08-11 (open weights, 22B, I2V + T2V + audio; ltx-2-5-fast/pro on LTX native API) — **NOT on AIMLAPI as of SC357 2026-09-13** (confirmed by web search — no AIMLAPI model string found in api-docs commits through Sept 13). |
...
| Seedance 2.0 Fast | `bytedance/seedance-2-0-fast` | TBD | **$0.316/sec ($1.58/5s — CONFIRMED 2026-06-30)** | **DO NOT USE** — 2.2× more expensive than Kling Pro ($0.728/5s — SC360 confirmed). Standard = $0.394/sec ($1.97/5s), also DO NOT USE. Face content-policy block risk. |
...
| **MiniMax H3 Max (Hailuo 3.0 Max)** | `minimax/h3-max` | 768p/2K est. | **~$0.104/sec AIMLAPI est.** ($0.08/sec native 768p × 1.3; 480p: ~$0.065/sec AIMLAPI est.) | ✓ ON AIMLAPI (SC357 2026-09-13 — added Aug 31 per api-docs commit). Released Sept 2, 2026 by MiniMax (post-trained from H3 via fal.ai co-release). T2V/I2V/first+last frame; 15s max. **DO NOT USE** for non-char shots — Hailuo 2.3 Fast wins at $0.0416/sec (2.5× cheaper at 768p). CANARY REQUIRED (audio disable param unconfirmed, no `generate_audio` equivalent found). Only viable if 15s+ duration specifically needed and quality justifies cost premium. |
...
| Sora 2 Standard I2V | `openai/sora-2-i2v` | 720p | **~$0.13/sec est.** (~$0.65/5s) | ⚠️ SUNSET Sept 24 2026 (**11 days remaining — SC357 2026-09-13. IMMINENT.** Confirmed by OpenAI. No replacement model announced). Audio ALWAYS generated — no disable param. DO NOT USE. |
| Sora 2 Standard T2V | `openai/sora-2-t2v` | 720p | **~$0.13/sec est.** (~$0.65/5s) | ⚠️ SUNSET Sept 24 2026 (11 days remaining — SC357 2026-09-13). Audio ALWAYS generated. DO NOT USE. |
...
**LTXV 2 Fast — ❌ BROKEN as of 2026-08-15 (SC262):** `ltxv/ltxv-2-fast` NOW ERRORS on AIMLAPI. Lightricks removed LTX-2 strings on Aug 15, 2026 as announced. AIMLAPI did not add `ltxv/ltxv-2-3-fast` before the deadline. Do NOT call this string — it will fail. Hailuo 2.3 Fast (`minimax/hailuo-2.3-fast`, $0.0416/sec) is the confirmed fallback for all non-character I2V 5s+ clips. When AIMLAPI adds the replacement string (expected: `ltxv/ltxv-2-3-fast`), run a canary to confirm. **Note:** LTX-2.5 released 2026-08-11 (22B, 4K, 50fps, native audio, I2V in 6.8s on GB200 superchips). **Still NOT on AIMLAPI as of SC357 2026-09-13** (confirmed: AIMLAPI api-docs commit audit Sept 13 shows no LTX-2.5 string added; LTX native API offers `ltx-2-5-fast` at $0.09/s 720p, $0.13/s 1080p and `ltx-2-5-pro` at $0.12/s 720p, $0.17/s 1080p, but AIMLAPI has not added any LTX-2.5 string). Third-party unconfirmed prices: $0.09/s 720p, $0.15/s 1080p. Watch for AIMLAPI model string (likely `ltxv/ltxv-2-5-fast`).
...
|-------|---------------|---------|--------|
| Meta Muse Image (CONFIRMED ON AIMLAPI — canary pricing still needed) | `meta/muse-image` | **~$0.013 AIMLAPI est.** ($0.01 native Meta API) | Multi-ref character composition, precise text, iterative refinement — 93% cheaper than NBP Edit; see section below. Also: `meta/muse-image-edit` for I2I editing. Both confirmed via GitHub api-docs commits Aug 2026. |
...
*Meta Muse Image (**CONFIRMED ON AIMLAPI** — SC311 2026-08-31, canary test still needed): Released July 7, 2026 by Meta Superintelligence Labs. Native Meta API: **$0.01/image** → AIMLAPI est. **~$0.013/image** (1.3× markup). **AIMLAPI availability: CONFIRMED** — GitHub api-docs audit (SC311 Aug 2026) shows two commits adding "Meta's Muse image models (generation and editing)". Model strings confirmed: `meta/muse-image` (T2I/multi-ref generation) and `meta/muse-image-edit` (I2I editing). Canary test still required to verify (1) exact AIMLAPI billing amount, (2) whether aspect_ratio: "9:16" is supported, (3) character identity cosine similarity vs NBP Edit. Capabilities: T2I + I2I + multi-reference composition; identity/character consistency when iterating on the same image; multi-ref composition (people, objects, clothing, styles, environments); precise text rendering within images; iterative reasoning (plans layout before drawing; anchored composition); agentic refinement via code/web tools. Cost position: **93% cheaper than NBP Edit ($0.195 → ~$0.013)** — if character consistency quality passes, this becomes the primary character DRAFT model. **Identity consistency note:** Muse Image is designed for "preserve subjects across edits" but uses a different mechanism than NBP Edit Subject Binding; no documented 0-100 strength slider. **Canary test:** (1) call `meta/muse-image` with character ref + 9:16 aspect_ratio + simple pose prompt; (2) check actual AIMLAPI billing (~$0.013 expected); (3) compare face cosine similarity vs NBP Edit baseline via InsightFace (PASS ≥ 0.62); (4) verify no audio generation risk (image model — no audio). If canary passes, route ALL character draft iterations here (save ~$0.18/iteration vs NBP Edit). Finals stay on NBP Edit until Muse Image identity lock is proven at production quality.*
...
**LTX-2.5 (SC311 recheck 2026-08-31):** Open weights + LTX native API + fal.run. **NOT on AIMLAPI as of 2026-08-31 (SC311 GitHub commit audit Aug 24-31 shows no LTX-2.5 addition).** Native pricing (CORRECTED SC311): `ltx-2-5-fast`: **$0.09/sec 720p, $0.13/sec 1080p, $0.19/sec 1440p, $0.30/sec 4K**; `ltx-2-5-pro`: **$0.12/sec 720p, $0.17/sec 1080p**. Previous pricing had 1080p at $0.15/sec — corrected to $0.13/sec for fast variant. When/if added to AIMLAPI, expected string: `ltxv/ltxv-2-5-fast` (CANARY REQUIRED). At native 720p fast ($0.09/s): 5s = $0.45 — more expensive than Hailuo 2.3 Fast ($0.208/5s). No cost advantage at 720p vs Hailuo 2.3 Fast. Features: multi-shot scene consistency, synchronized audio generation (I2V in 6.8s on GB200), supports T2V + I2V + audio-to-video. Generate audio is off by default at LTX native API. Watch for AIMLAPI string.
...
59. **SC336 STATUS REFRESH (2026-09-07 — cost optimization study, pass 44):** (a) **Wan 3.0 audio param — SC329 conflict REINFORCED.** Independent research via Runware docs and Alibaba Cloud documentation confirms the Wan 3.0 native I2V endpoint uses `enable_audio` (boolean, default true). The parameter `generate_audio` does NOT appear in Alibaba's own Wan 3.0 API docs — only in AIMLAPI reseller documentation (SC315 sources: Segmind, CometAPI, EvoLink, etc.). Conclusion: AIMLAPI likely normalizes `generate_audio` → `enable_audio` internally, but this is UNCONFIRMED. Safe approach (from SC329) VALIDATED: always send BOTH `generate_audio: false` AND `enable_audio: false` in the same payload. Audio toggle has NO cost impact (same billing either way). Table note updated with SC336 reinforcement. (b) **Sora 2 sunset — 17 days remaining (Sept 24, 2026).** Confirmed complete discontinuation: no replacement model announced. Already marked DO NOT USE. Table countdown updated. (c) **Wan 3.0 Alibaba discount expires Sept 23 (last full day — 16 days remaining).** Active through Sept 23, gone on Sept 24. Applies to Alibaba Cloud/Qwen ONLY, NOT AIMLAPI. Table note updated with correct "last full day" framing. (d) **LTX-2.5 still NOT on AIMLAPI (Sept 7 recheck).** LTX native API offers `ltx-2-5-fast` ($0.09/s 720p) and `ltx-2-5-pro` ($0.12/s 720p) but no AIMLAPI string confirmed. AIMLAPI docs not fetchable directly. No GitHub api-docs commit for LTX-2.5 visible in Sept 2026 commits. Status note updated from SC322 date to SC336. (e) **Kling 3.0 ≠ new model.** Third-party references to "Kling 3.0" at $0.075/sec refer to EvoLink/other non-AIMLAPI providers. AIMLAPI naming for the same generation is `klingai/video-v3-*`. No new Kling model confirmed on AIMLAPI. (f) **No new AIMLAPI video models confirmed Sept 5-7.** Sept 1-7 GitHub commits show only text/LLM model additions (GPT-6-Astra, Claude Fable 5.1, Gemini 3.8 Flash, Qwen 3.8 Flash). All existing routing decisions unchanged.

58. **SC315 STATUS REFRESH (2026-09-01 — cost optimization study, pass 41):** (a) **Wan 3.0 audio param CONFIRMED: `generate_audio: false`.** Multiple sources (Segmind, CometAPI, EmpirioLabs, EvoLink, pixazo.ai) converge: Wan 3.0 uses boolean `generate_audio: false` to disable audio — NOT `audio_mode: "mute"` (that is Wan 2.7 convention). CRITICAL: audio defaults ON in Wan 3.0 — **MUST send `generate_audio: false` for Shari'ah compliance.** Importantly, disabling audio does NOT change the price (same billing either way — no surcharge, no discount). Table row updated from "UNCONFIRMED" to ✓ CONFIRMED. (b) **Wan 3.0 30% launch discount: Alibaba Cloud / Qwen Cloud ONLY, NOT AIMLAPI.** Official X post from @Alibaba_Wan states discount applies "on Alibaba Cloud Model Studio and Qwen Cloud from August 23 to September 23." No evidence discount passes through to AIMLAPI. Existing docs note already correct — no change needed. (c) **No new AIMLAPI video models added Sept 1, 2026.** GitHub api-docs: only one commit (GITBOOK-1171, Sept 1 — documentation update, no model content). Last substantive additions remain Aug 27-31 batch: Meta Muse Image, Wan 3.0, Tencent HY-MT2/HY4-preview, Gemini Omni 1.1 Flash. (d) **LTX-2.5 still NOT on AIMLAPI (Sept 1 recheck).** Native API: `ltx-2-5-fast` ($0.09/s 720p, $0.13/s 1080p), `ltx-2-5-pro` ($0.12/s 720p, $0.17/s 1080p). Open weights on HuggingFace. No `ltxv/ltxv-2-5-fast` or `ltxv/ltxv-2-3-fast` string found on AIMLAPI. Hailuo 2.3 Fast remains sole confirmed non-char I2V. (e) **FLUX 3 Video still NOT on AIMLAPI (Sept 1 recheck).** Available on fal.ai (`blackforestlabs/flux-3`); BFL native API. Audio always generated in FLUX 3 — strip required. Draft mode $0.06/sec native, ~$0.078/sec est. AIMLAPI. Still not on AIMLAPI. (f) **Kling v1/v2 retirement (Sept 15, 2026 — 14 days from today).** No official Kuaishou announcement found in search; AIMLAPI docs still show v1.6 page. Pipeline already uses only v3 strings — no action required. Monitor AIMLAPI for v1/v2 string errors after Sept 15. (g) **All other routing decisions unchanged.** Wan 3.0 30% promo discount expires Sept 23 (Alibaba only), Kling Pro ($1.46/5s) + Hailuo 2.3 Fast ($0.0416/sec) remain active production routes.
