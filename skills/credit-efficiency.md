...
28. **Wan 2.7 R2V (`alibaba/wan-2-7-r2v`) STATUS UPGRADED (2026-06-30): Now listed in AIMLAPI docs navigation index.** Previously "Coming Soon" — now appears as a named entry in the docs index. Dedicated indexed page not yet crawled by search (index lag expected). Status: LIKELY LIVE — CANARY REQUIRED immediately. Expected AIMLAPI price: ~$0.10/sec (same as Wan 2.7 I2V/T2V). Third-party Segmind flat rates ($0.625/720p, $0.9375/1080p) are Segmind-only, NOT applicable here. Capabilities: up to 5 mixed refs (images + video clips + audio); characters referenced as `Image1`, `Image2`, `Video1` slot names in prompt; 720p/1080p, up to 10s. CANARY: call with 2 character hero frame refs + `aspect_ratio: "9:16"`, record actual AIMLAPI cost and face retention. If passes → high-priority draft routing; could significantly reduce Kling Pro character shot costs. Fallback until canary passes: Wan 2.6 R2V (`alibaba/wan-2-6-r2v`) is confirmed live.
...
43. **Seedance 2.5 — Future watch, not yet on AIMLAPI (2026-06-30).** Announced June 23, 2026 at Volcano Engine FORCE conference. Enterprise beta now; public launch targeted early July 2026. Key specs: 30-second native clips (no stitching), up to 50 multimodal references, 4K, co-generated synchronized audio. API pricing not yet disclosed. Volcano Engine API must open before third-party (AIMLAPI) integration. Legal note: Hollywood C&D in Feb 2026 led ByteDance to add content filters blocking recognizable real faces — less relevant for our branded character. Monitor for AIMLAPI docs page in July 2026. Do NOT canary until AIMLAPI page appears.
