...
| **Kling O1 Image-to-Video (CANARY)** | `klingai/video-o1-image-to-video` | 1080p (9:16) | **$0.59** ($0.118/sec — CONFIRMED SC194) | **$0.35** (3s) — Standard I2V, tag-based subject consistency, up to 7 refs. 46% cheaper than Kling v3 Standard ($1.09/5s). CANARY REQUIRED — verify identity vs NBP Edit baseline before routing character drafts here. |
...
6. **R2V (STATUS REASSESSED 2026-07-09):** Rule 46 confirmed "Coming Soon" on 2026-07-06. As of 2026-07-09, AIMLAPI docs navigation shows the R2V page entry (suggesting it may now be callable). Before calling, verify a dedicated docs page exists at `docs.aimlapi.com/api-references/video-models/alibaba-cloud/wan-2.7-reference-to-video`. When confirmed callable: canary with `image_urls: [hero_frame_url_1, hero_frame_url_2]`, `aspect_ratio: "9:16"`, `duration: 5` (max is 10s for R2V — NOT 15), prompt referencing `Image1`. Verify face retention vs NBP Edit baseline. Note: parameter is `image_urls` (array), NOT `image_list`. **R2V duration cap: 2-10 seconds (NOT 2-15 — that limit applies to I2V/T2V only).**
