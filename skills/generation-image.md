...
- **⚠️ NB2 blockReason OTHER now identical to NBP (2026-07-17 — CRITICAL FOR DRAFT WORKFLOW):** As of March 2026, NB2 (`google/nano-banana-2`) has the SAME 8-category blockReason OTHER policy as NBP Edit — NOT the lighter pre-NB2 policy. The 4 original categories (NSFW, watermark removal, copyrighted IP, minors) PLUS the 4 new March 2026 categories (public figures/celebrities, financial information modification, **outfit/face swapping — more aggressively detected in NB2 than NBP**, implicit suggestive content). **Impact on our draft iteration workflow:** If you use NB2 to iterate character compositing calls (character sheet + new scene), you may hit the same outfit/face-swapping block as NBP — the model-switch fallback in step (5) above is therefore not reliable as a universal bypass; it works only when the NBP block is on a category where NB2 enforcement differs. For character draft iteration that keeps hitting OTHER: prefer **Seedream 5.0 Pro** ($0.06, 10 refs, confirmed on AIMLAPI — canary first) or **T2I with text description**. Do NOT burn NB2 credits retrying a call blocked for outfit/face-swapping.
