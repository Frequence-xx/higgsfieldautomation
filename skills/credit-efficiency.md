...
|-------|---------------|-----------|---------------------|----------|
| Kling v3 Std Turbo I2V (canary) | `klingai/video-v3-standard-turbo-image-to-video` | 720p (9:16) | **$0.73** ($0.146/sec est.) | **$0.44** — Last frame OPTIONAL (confirmed multiple providers, June 2026). ⚠️ Audio always generated — strip required. CANARY before routing drafts here. |
| Kling v3 Std Turbo T2V (canary) | `klingai/video-v3-standard-turbo-text-to-video` | 720p (9:16) | **$0.73** ($0.146/sec est.) | **$0.44** — T2V variant. Same audio/cost behavior as I2V Turbo. CANARY REQUIRED. |
| Kling v3 Turbo Pro I2V (canary) | `klingai/video-v3-turbo-pro-image-to-video` | **1080p (9:16)** | **$0.91** ($0.182/sec — AIMLAPI pricing page confirmed June 2026) | **$0.546** (3s) — 37% cheaper than v3 Pro ($1.46). ⚠️ Audio always generated — strip required. Last frame OPTIONAL. CANARY REQUIRED before routing finals here. |
| Kling v3 Turbo Pro T2V (canary) | `klingai/video-v3-turbo-pro-text-to-video` | **1080p (9:16)** | **$0.91** ($0.182/sec) | T2V variant at 1080p. Same audio/cost as Turbo Pro I2V. CANARY REQUIRED. |
...
**⚠️ AUDIO ALWAYS GENERATED on Turbo models** — same as multi-shot mode. Kling 3.0 Turbo has "native audio included" in its base pricing. Whether `generate_audio: false` suppresses audio at AIMLAPI is UNCONFIRMED. Treat like multi-shot: **strip audio immediately on every download, before ANY QA playback:** `ffmpeg -i input.mp4 -an -c:v copy output_silent.mp4`. Do NOT rely on `generate_audio: false` to protect Shari'ah compliance on Turbo models until canary confirms.

**Turbo Pro final-pass savings (if canary passes):** 1 character + 1 truck final: saves ~$1.10/video ($1.46 → $0.91 × 2 clips). Requires InsightFace cosine similarity ≥ 95% vs v3 Pro baseline before routing finals here.
...
**Status (2026-05-26, updated 2026-06-21):** Two variants confirmed on AIMLAPI.
...
**`generate_audio: false` CONFIRMED (2026-06-21):** The `generate_audio` parameter IS accepted by Seedance 2.0. Setting it to `false` disables audio generation. **Price is the SAME whether audio is on or off** — no credit savings from disabling. Critical for Shari'ah compliance: always pass `"generate_audio": false` in any Seedance canary call. Audio strip (`ffmpeg -i input.mp4 -an -c:v copy`) is no longer required IF `generate_audio: false` is confirmed to work at AIMLAPI layer — verify in canary.

**Face content policy — CONFIRMED SEVERE (2026-06-21):** ByteDance blocks any reference image containing a detectable REAL human face (deepfake/IP protection). What gets blocked: photographs of real people. What passes: AI-generated portraits, illustrated characters, 3D renders, stylized faces, side profiles. **Our NBP Edit hero frames are AI-generated portraits — they may pass the filter.** This is the critical canary question: do AI-generated character sheets trigger Seedance's face detection? The 3 prior blocks on character sheets may have been due to photorealistic rendering triggering face detection, not the AI-origin itself. Verify in canary before ruling out.
...
1. Submit one 5s I2V call with `bytedance/seedance-2-0-fast`, NBP Edit hero frame (AI-generated portrait), `aspect_ratio: "9:16"`, `"generate_audio": false`
2. Check actual `usd_spent` from response — confirm ~$0.91. Confirm audio is absent from the downloaded file.
3. Run brand binary checklist + Shari'ah compliance check. Monitor for face content-policy blocks.
4. If content-policy block occurs → try again with a side-profile or partially cropped reference (reduce facial detail) — ByteDance's filter is face-detection-based, not AI-origin-based.
5. If passes → Seedance 2.0 Fast is viable for character drafts. Note: `generate_audio: false` confirmed same-price on Seedance (unlike Kling where audio adds cost).
...
28. **Wan 2.7 R2V (`alibaba/wan-2-7-r2v`) is NOT confirmed on AIMLAPI as of 2026-06-21 (re-checked today).** Listed as "Coming Soon" in AIMLAPI model database. `site:docs.aimlapi.com` search returns only Wan 2.6 R2V page — NO Wan 2.7 R2V page. Use `alibaba/wan-2-6-r2v` for multi-ref character work requiring R2V. When Wan 2.7 R2V lands: parameters accept up to 5 mixed refs (images + video clips); characters referenced in prompt as `Image1`, `Image2`, `Video1` slot names.
...
38. **Kling v3 Turbo Pro I2V (`klingai/video-v3-turbo-pro-image-to-video`) is CONFIRMED ON AIMLAPI pricing page at $0.182/sec = $0.91/5s (2026-06-19).** Resolution: 1080p. 37.7% cheaper than Kling v3 Pro ($1.46). Last frame is OPTIONAL — single-frame I2V supported. ⚠️ Audio always generated (same as multi-shot) — strip immediately: `ffmpeg -i input.mp4 -an -c:v copy output_silent.mp4`. Do NOT use for finals until canary verifies: (1) actual AIMLAPI cost, (2) 1080p output confirmed, (3) InsightFace cosine ≥ 95% vs v3 Pro baseline, (4) audio strip confirmed sufficient for Shari'ah compliance. Standard Turbo I2V ($0.73/5s, 720p) has same audio risk. Turbo Pro T2V also confirmed at $0.182/sec. Kling O3/Omni still NOT on AIMLAPI.
39. **Kling v3 Turbo Element Binding (Subject Binding) IS SUPPORTED (confirmed 2026-06-21).** Official Kling docs and community testing confirm that the Element Reference / Bind Subject feature works on Kling 3.0 Turbo (both Standard Turbo and Turbo Pro). The same `elements` array used for v3 Pro applies to Turbo models. Turbo described as "strong prompt adherence, stable motion" — identity lock is architecturally intact. This means Turbo Pro IS a viable candidate for character finals once cosine similarity canary passes (InsightFace ≥ 0.92 vs v3 Pro baseline). Until canary is run, Turbo is draft-only; v3 Pro remains production standard.
40. **Hailuo 3.0 (MiniMax) is NOT on AIMLAPI as of 2026-06-21.** Exists as a product: native 4K, up to 30s clips, integrated spatial audio, advanced character persistence — but only available via fal.ai and MiniMax native API. Since our pipeline is AIMLAPI-only (Farouq directive 2026-04-16), Hailuo 3.0 is NOT actionable. Only Hailuo 02 ($0.0728/sec — DO NOT USE, too expensive) and Hailuo 2.3 Fast ($0.0416/sec, I2V only) remain the AIMLAPI MiniMax options. Monitor for future AIMLAPI addition.
41. **Veo 3.1 Fast pricing CONFIRMED at source (2026-06-21): $0.10/sec audio-OFF at Vertex AI = ~$0.13/sec on AIMLAPI.** Multiple sources confirm: $0.15/sec with audio, $0.10/sec without (33-40% audio savings). AIMLAPI ~1.3× markup → $0.13/sec audio-OFF = ~$0.65/5s. Our existing docs were already correct. `generateAudio: false` MUST be passed (camelCase, same as Veo 3.1 Lite). No new action required.
