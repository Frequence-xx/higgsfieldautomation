...
| whisper.cpp | **v1.9.4 (2026-09-11)** | Released today. GGML sync to v0.23.0 (was v0.20.2 in v1.9.3); decoder reseeding between calls fixed (consistent behaviour across multi-call sessions); language detection: encoder callback now fires before language auto-detect; server `/detect` endpoint now returns language field in response. Windows ARM + CUDA 13.4.1 support. **No word-timestamp changes — caption pipeline unaffected.** Build via cmake from v1.9.4 tag (no pre-built binary). See SC351 update. |
...
**SC358 update (2026-09-13):** Post-production topic (pass 46). **One substantive change: Remotion v4.0.524 released today. All other tools confirmed unchanged.**

**Remotion v4.0.524 released 2026-09-13** — 4 days after v4.0.523 (2026-09-09). Key changes: (1) **`toneFrequency` pitch-shifting now works in both preview AND rendering** — previously pitch-shifting was a preview-only feature; renders silently ignored `toneFrequency`. For our pipeline, audio mixing is done via FFmpeg (not Remotion's `<Audio>`), so this is low-impact unless a future composition uses `<Audio>` components with pitch-corrected voiceover. (2) **Mediabunny upgraded to 1.56.1** — minor version bump to the underlying media engine; no pipeline API changes. (3) **Composition management no longer requires Prettier** — Studio can create, duplicate, rename, and delete compositions without Prettier installed; minor DX improvement. (4) Studio: syntax highlighting in error overlays; keyboard shortcuts displayed in toolbar tooltips; timeline children collapse without visual flicker. (5) Rspack bundles now work without optional Studio dependencies. No new `@remotion/effects` or `@remotion/transitions` additions. **Pipeline action:** Upgrade with `npm install remotion@latest` when next building Remotion compositions. If any composition uses `<Audio toneFrequency={...}>`, that pitch correction will now be baked into renders — verify output is correct after upgrading.

**All other tools confirmed unchanged (2026-09-13):** FFmpeg 9.0.1 (no 9.0.2), whisper.cpp v1.9.4 (no v1.9.5), SVT-AV1 v4.2.0 (no v4.3), rife-ncnn-vulkan CLI v20250112 (no new binary — Arch AUR confirms 20250112 still current), PySceneDetect v0.7.1 (no v0.7.2), Practical-RIFE v4.26 (no v4.27, v4.25 remains pipeline default for diffusion video).

**SC351 update (2026-09-11):** Post-production topic (pass 45). **One substantive change: whisper.cpp v1.9.4 released today. All other tools confirmed unchanged.**

**whisper.cpp v1.9.4 released 2026-09-11** — 22 days after v1.9.3 (2026-08-20). Key changes: (1) **GGML sync to v0.23.0** (was v0.20.2 in v1.9.3) — extensive backend optimizations for CUDA, Metal, Vulkan, SYCL; faster GPU transcription on NVIDIA/AMD/Apple Silicon/Intel Arc. (2) **Decoder reseeding fix** — "re-seed decoder 0 between calls" ensures consistent inference across multi-call batching sessions; fixes subtle accuracy divergence when processing multiple clips in sequence (e.g., batch-transcribing a session's voiceovers). (3) **Language detection improvement** — encoder callback fires before language auto-detection; gives pre-processing hooks better control over the detection pipeline. (4) **Server `/detect` endpoint now returns language field** — relevant if using whisper.cpp server mode for Dutch language detection. (5) Infrastructure: removed leftover Metal kernel files; Windows ARM + CUDA 13.4.1 support added. **No word-timestamp changes** — our `--word-timestamps true` caption pipeline is unaffected. **Pipeline action:** Upgrade from v1.9.3 → v1.9.4 when next rebuilding the transcription step. Build via cmake from the v1.9.4 tag (no pre-built binary available — same build path as v1.9.3). The decoder reseeding fix is the most practically relevant change: any session that calls whisper.cpp multiple times in sequence (batch voiceover transcription) will get more consistent word-level timestamps across clips.

**All other tools confirmed unchanged (2026-09-11):** FFmpeg 9.0.1 (no 9.0.2), Remotion v4.0.523 (no v4.0.524 — confirmed GitHub releases page), SVT-AV1 v4.2.0 (no v4.3 — confirmed GitLab), rife-ncnn-vulkan CLI v20250112 (no new binary — confirmed GitHub), PySceneDetect v0.7.1 (no v0.7.2), Practical-RIFE v4.26 (no v4.27, v4.25 remains pipeline default for diffusion video).

**SC344 update (2026-09-09):** Post-production topic (pass 44). **Two substantive Remotion changes. All other tools confirmed unchanged.**

**Remotion advanced to v4.0.523 (released 2026-09-09).** Two releases since SC337. (1) **v4.0.522 (Sept 7, 2026):** Studio-only — timeline zoom to audio asset previews, composition menu in title bar, sequence "Split clip" context menu, sidebar restoration via drag. No new @remotion/effects, no pipeline impact. (2) **v4.0.523 (Sept 9, 2026):** **`tear()` added to `@remotion/effects`** (§11q — WebGL2 progressive ripping animation); **`blurSlide()` added to `@remotion/transitions`** (§11q — blur-based slide transition); **New `@remotion/video-matting` package** (§11q — video background matting/removal with Studio integration, low pipeline priority for current ads); **`@remotion/media` audio-gap-after-pause fix** (resolves audio gaps after interactive pause — upgrade if any Remotion composition uses pause/resume). Studio: keyboard shortcut customization, caption JSON import from public folders, sequence reordering controls, Lambda overwrite permissions fix. 54 commits, 6 contributors.

**All other tools confirmed unchanged (2026-09-09):** FFmpeg 9.0.1 (no 9.0.2), SVT-AV1 v4.2.0 (no v4.3), rife-ncnn-vulkan CLI v20250112 (no new binary), PySceneDetect v0.7.1 (no v0.7.2), Practical-RIFE v4.26 (no v4.27, v4.25 remains pipeline default for diffusion video), whisper.cpp v1.9.3 stable (SC337 correction confirmed — v1.9.4 subsequently released 2026-09-11, see SC351).

**SC337 update (2026-09-07):** Post-production topic (pass 43). **One substantive change. All other tools confirmed unchanged.**

**whisper.cpp v1.9.3: SC305 "pre-release" finding CORRECTED — v1.9.3 is now STABLE.** Evidence: (1) Arch Linux packages `whisper-cpp 1.9.3-1` in the main stable repository (Arch never packages pre-releases in main); (2) GitHub release page `github.com/ggml-org/whisper.cpp/releases/tag/v1.9.3` exists; (3) freedom.tech post dated 2026-08-20 covers v1.9.3 as a full release. Conclusion: v1.9.3 was released stable on **2026-08-20** — SC305 (2026-08-29) was incorrect about its pre-release status. **Changes in v1.9.3:** GGML 0.19 → 0.20.2 sync — CUDA/SYCL kernel fusing (faster GPU transcription), Metal TQ2_0 support (Apple Silicon speed gain), ARM/WASI fixes; security patches. **No word-timestamp changes** — caption pipeline unaffected. **Pipeline action:** Upgrade whisper.cpp from v1.9.2 to v1.9.3 when next building or deploying the transcription step. Build via cmake (v1.7.3+ requires cmake — no pre-built binary download; cmake is available in the pipeline environment).

Note on `@remotion/install-whisper-cpp`: Remotion's install package recommends v1.5.5 and has no Windows binary for v1.9.x. Our pipeline uses whisper.cpp CLI on Linux server (cmake build) — unaffected by Remotion's binary limitation. Continue using whisper.cpp v1.9.4 CLI directly per `captions-and-titles.md`. (updated SC354 Sep 12, 2026)

**All other tools unchanged (2026-09-07 check):** FFmpeg 9.0.1 (no 9.0.2), SVT-AV1 v4.2.0 (no v4.3), rife-ncnn-vulkan CLI v20250112 (no new binary), PySceneDetect v0.7.1 (no v0.7.2), Practical-RIFE v4.26 (no v4.27). Remotion v4.0.521 (confirmed Sept 5 per SC330) — no new release in past 2 days.
...
### 11q. New in v4.0.523 (2026-09-09): `tear()`, `blurSlide()`, `@remotion/video-matting`

#### `tear()` — Progressive Ripping Animation (`@remotion/effects`, v4.0.523)

**`tear()`** (added v4.0.523, 2026-09-09) — WebGL2 effect that simulates a progressive paper-tear reveal across the frame. Animates a tearing motion from one edge that progressively removes/reveals the layer beneath.

**Pipeline relevance for Snelverhuizen:**
- Scene-to-scene transitions where a dramatic "ripping away" motion suits the concept (e.g., stress-of-moving → relief reveal)
- Title card entrance animation — tear away to reveal the headline
- **Never on character close-up or truck shots** — distorts frame geometry

**Import:**
```tsx
import { tear } from "@remotion/effects";
// Part of @remotion/effects — already installed with other effects
```

**Timing guidance:** Drive `progress` from `1` (torn away) → `0` (full reveal) over 12–18 frames for a clean entrance. Easing: `spring({ fps, frame, config: { damping: 200 } })` prevents overshoot.

**When to use:**
- Emotional transitions (stress → relief, before → after)
- End card / CTA dramatic entrance
- Abstract interstitial / title cards

---

#### `blurSlide()` — Blur Slide Transition (`@remotion/transitions`, v4.0.523)

**`blurSlide()`** (added v4.0.523, 2026-09-09) — new presentation for `@remotion/transitions`. Slides two compositions together while applying a progressive blur during the transition window — cinematic motion-blur slide cut.

**Pipeline relevance:**
- Cleaner alternative to a plain cut between B-roll shots
- End card → CTA slide transitions with cinematic feel

**Usage:**
```tsx
import { TransitionSeries, linearTiming } from "@remotion/transitions";
// Import path TBC — verify with `npm view @remotion/transitions` on first use
// Likely: import { blurSlide } from "@remotion/transitions";
...
**Note:** Verify the exact import subpath from `@remotion/transitions` on first implementation — v4.0.523 release notes do not specify the import path for `blurSlide()`.

---

#### `@remotion/video-matting` — Video Background Matting (v4.0.523, NEW PACKAGE)

**New package** introduced in v4.0.523. Provides AI-based video matting (background removal/replacement) inside Remotion compositions, with Studio UI integration for processing video assets.

**Pipeline relevance for Snelverhuizen:**
- **LOW priority for current pipeline** — our AI video generation (Kling/Veo) produces controlled backgrounds natively; we rarely need background matting.
- **Potential cost-saving use:** If a character clip has a distracting background that causes a QA fail, matting + compositing onto a clean background could avoid a $1.09–$1.46 regeneration. Evaluate on a case-by-case basis.
- **Install:** `npm install @remotion/video-matting`
- **Canary required:** Full API and matting model quality not yet externally verified. Test on a Kling clip before relying on it as a retry alternative.
...
- [ ] Remotion version recheck (SC344, 2026-09-09): latest is **v4.0.523** (released Sept 9, 2026). v4.0.522 (Sept 7): Studio-only, no new effects. v4.0.523 (Sept 9): **`tear()` added to @remotion/effects** (§11q — progressive ripping animation); **`blurSlide()` added to @remotion/transitions** (§11q); **`@remotion/video-matting` new package** (§11q — low pipeline priority); `@remotion/media` audio-gap-after-pause fix. Upgrade to v4.0.523 when next touching Remotion compositions.
