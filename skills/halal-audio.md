...
**Note:** Pronunciation dictionary phoneme rules work with `eleven_v3` and `eleven_flash_v2` only — not with `eleven_multilingual_v2`. For standard production (eleven_v3), create the dictionary once and attach it per call. For draft passes (Flash v2.5), `apply_text_normalization="on"` is **confirmed Enterprise-only** (SC313 Sept 1, 2026 — see Known Issues) — use the pronunciation dictionary as a fallback on non-Enterprise plans.

**⚠ Pronunciation dictionary archive check (SC274, Aug 2026):** Pronunciation dictionaries can be archived. An archived dictionary silently stops applying its rules — `convert()` calls using an archived dict_id succeed without error but pronunciation rules are NOT applied. Before each production session, verify the stored dict_id is still active:
```python
# List only NON-archived dictionaries (include_archived=False)
dicts = client.pronunciation_dictionaries.list(include_archived=False)
active_ids = {d.id for d in dicts.pronunciation_dictionaries}
if PRONUNCIATION_DICT_ID not in active_ids:
    raise RuntimeError("Snelverhuizen pronunciation dictionary was archived — recreate it (see §0 creation block above)")
# Also check archive_time field directly:
# d.archive_time is None → active; not None → archived Unix timestamp
```
Add this check to the pre-generation script. If the dictionary is archived, recreate it using the `create_from_rules()` block above and store the new ID in project config.
...
**`voice_library_removal_notice` webhook (SC274, Aug 2026):** ElevenLabs now fires a `voice_library_removal_notice` webhook event when a Voice Library community voice is scheduled for removal — giving advance warning before the voice_id becomes invalid. If a webhook endpoint is configured for this pipeline in the future, subscribe to this event type to get early warning of Willem removal without needing to poll the voices API manually. For now, the per-session `recording_quality` check above (GET /v1/voices?search=Willem) remains the safeguard.
...
| **Aswati** — aswati.co/background-nasheed | Aswati Content License (royalty-free, in-house production) | Yes — commercial, no Content ID claims | No | Free: 8 tracks with email only (no card). Studio: $9/mo for 90+ tracks with new monthly drops (updated Aug 2026; up from 70+ confirmed July 2026). In-house production = zero third-party copyright claims. **⚠ Library contains both pure a cappella AND voice+percussion tracks** — not all are instrument-free. Always select tracks explicitly labelled "vocals only" and run nasheed_check.py (§9) before use. Daf/percussion tracks = REJECT per Snelverhuizen policy. |
| **NoorLoops** — noorloops.com/en/halal-music | NoorLoops Commercial License | Yes — worldwide commercial license covering YouTube, Reels, TikTok, podcasts, ads, and brand campaigns. No per-play royalties. Content ID dispute support included. | No | Subscription (tiered — check noorloops.com for current pricing). **Every track is tagged by Islamic suitability tier (music library uses these labels): Voice-only/Ambience-only (no instruments), Percussion-light (subtle duff-inspired rhythm), Broad-use (may contain instruments).** For Snelverhuizen: ONLY use tracks tagged **Voice-only/Ambience-only** — these are pure vocals/nasheed with zero instruments. Percussion-light and Broad-use = REJECT per "no instruments" policy. Run nasheed_check.py (§9) before use. Confirmed August 2026 — new dedicated halal-aware library; licensing includes Content ID dispute support (differentiator over YouTube channels). **Note: NoorLoops SFX library (§2) uses different tier labels: Conservative/Moderate/Broad — do not confuse the two.** |
| **Lomeyo Nasheed Directory** — nasheed.lomeyo.com | MIT (code) / CC variants per track (CC0, CC-BY, CC-BY-SA; `attribution_text` field carries exact credit string) | Yes — all tracks "freely licensed for commercial reuse". CC-BY/CC-BY-SA require attribution (copy `attribution_text` into video description). CC0 tracks need none. | Per track (check `license` field via API) | Public reads: GET `/api/public/` — no key required. Full access: free API key at `/api/v1/keys`. MCP tools: `pick_background_track`, `search_nasheeds`, `get_nasheed`. Filters: `instrumentation`, `license`, `mood`, `language`, `tags`, `duration`. **PURPOSE-BUILT FOR AI AGENTS** — first machine-queryable halal catalog with instrumentation metadata (no ear-verification needed). **⚠ Catalog includes voice+duff tracks** — the rubric allows frame drum. Filter `instrumentation=voice_only` to get pure acappella tracks. NEVER use tracks without instrumentation=voice_only filter — duf/percussion = REJECT per Snelverhuizen policy. Run nasheed_check.py (§9) as secondary screen. Stricter scholarly position applied — tracks passing rubric are acceptable to all scholarly views. Code: github.com/lomeyollc/nasheed-directory. **CANARY needed** — confirmed from GitHub/MCP listing (June 30, 2026); API response format and actual track quality require a $0 canary before production. |
...
### Tier 1a2: NoorLoops Islamic SFX (Halal-Certified — Brand-Specific Sounds)

- **URL:** `noorloops.com/en/islamic-sounds`
- **License:** NoorLoops Commercial License — worldwide, YouTube/Reels/TikTok/podcasts/ads/brand campaigns. No per-play royalties. Content ID dispute support.
- **Best for:** Islamic-branded ambient pads, adhan-inspired textures, vocal stabs, risers, transitions — sounds specifically designed for Muslim-brand video content that Pixabay/Mixkit don't cover
- **Instrument-presence tagging:** Conservative (no instruments) / Moderate (percussion-light) / Broad (may include instruments). **ONLY use Conservative-tagged SFX** for Snelverhuizen — reject Moderate and Broad
- **Note (Aug 2026):** New dedicated halal SFX library. Use when Pixabay/Mixkit/ElevenLabs SFX v2 return generic results for Islamic-branded transitions or ambient textures. Requires subscription — check noorloops.com for current pricing before first use.
...
| `pcm_8000` | Telephony PCM | Pro+ | 8kHz uncompressed — telephone quality; not suitable for video mixing. Present in AllowedOutputFormats (verified v2.59.0, unchanged through v2.64.0); use only for telephony/streaming integrations. |
...
**`music_v2` (June 2026 UI default; fully live on API as of Sept 2026):** music_v2 is the default model in the ElevenLabs UI and introduces chunk-based composition plans (`GenerationChunk` / `AudioRefChunk` segments). Use `model_id="music_v2"` on Generate music, Stream music, Generate music detailed, and Upload music endpoints to call it explicitly. **Aug 31, 2026:** `with_waveform_visual` parameter added to music endpoints — confirms music_v2 is actively receiving API updates. It still generates music with instruments — no halal advantage over music_v1. Treat identically: **DO NOT USE either `music_v1` or `music_v2` in this pipeline. The explicit model_id is documented here only to help AVOID accidentally using it — never pass it.**
...
**ffmpeg-normalize version history (pipeline-relevant — latest: v1.42.0, September 2, 2026):**
...
- v1.42.0 (September 2, 2026): New `--write-encoder-settings` option tags each normalized audio stream with its encoder settings (useful for debugging re-encode history). Subprocess output typing fix in CI. No API or loudness-target changes — all existing ffmpeg-normalize commands in §4f remain valid. Linux pipeline is unaffected.
...
| FFmpeg whisper filter — NOT a speech enhancer | whisper filter does ASR transcription only (outputs SRT/JSON) | Do not use as audio enhancement. For voiceover post-processing, continue with arnndn/afwtdn/dynaudnorm/loudnorm/deesser as documented. No new speech enhancement filters added in FFmpeg 8.x or 9.x. **FFmpeg 9.0.1 is the current stable release as of 2026-08-12** (major version "Lei", confirmed in post-production.md SC256 update 2026-08-14). Breaking change in 9.0: `tls_verify` defaults to 1 — no pipeline impact (AIMLAPI CDN certs are CA-signed). All audio filter commands (loudnorm, dynaudnorm, sidechaincompress, arnndn, afwtdn, deesser, dialoguenhance) are unchanged and valid on 9.0.1. |
| Pronunciation dictionary rules silently not applied after previously working | Dictionary was archived — `convert()` succeeds but archived dict_id is ignored with no error | Before each session, call `client.pronunciation_dictionaries.list(include_archived=False)` and verify stored PRONUNCIATION_DICT_ID is in the result. If missing: recreate with `create_from_rules()` and update stored ID (see §0). Do NOT rely on no error = working rules. |
...
| Want highest-quality TTS master audio (lossless) | eleven_v3 native rate is 44.1kHz; pcm_48000 upsamples TTS; ultra_lossless is Studio-only and not a TTS output_format | Use `pcm_44100` OR `wav_44100` for lossless TTS masters — both natively match eleven_v3's 44.1kHz rate (Pro tier required for both). `wav_44100` is a containerised WAV wrapper around the same PCM data — more convenient for non-FFmpeg tools (no `-f s16le` flags needed to decode). `pcm_44100` remains valid for FFmpeg pipelines. TTS uses `TextToSpeechConvertRequestOutputFormat` (not `AllowedOutputFormats`) — WAV formats confirmed in this type (added Jan 2026). Use `pcm_48000` for lossless SFX v2 masters (native 48kHz) — WAV formats are still NOT in `AllowedOutputFormats` used by SFX v2 endpoint. Never use `ultra_lossless` as output_format — it is a Studio Podcasts quality_preset, not available on TTS endpoint. |
...
Use ElevenLabs Scribe v2 to verify that a generated voiceover matches the intended script and to extract word-level timestamps for caption alignment. Scribe v2 achieves ≤5% WER on Dutch — significantly more accurate than Whisper base on Dutch. GA since March 11, 2026: 99 languages, 98% speaker label accuracy. SDK recheck 2026-08-15: v2.64.0 current; batch STT API unchanged since v2.59.0; `timestamps_granularity` accepts `"word"`, `"character"`, or `"none"`; word response includes `logprob` and `characters` fields.

**New realtime model IDs (SDK v2.63.0, August 11, 2026 — realtime WebSocket only, NOT for VO QA):** `scribe_v2_realtime_turbo` and `scribe_v2_realtime_lite` were added alongside the original `scribe_v2_realtime`. The batch endpoint (`client.speech_to_text.convert()`) still uses `model_id="scribe_v2"` — unchanged. New realtime params: `secondary_languages` (additional language list for code-switching detection), `filter_background_audio` (bool — suppress background noise before STT). These realtime params do NOT apply to batch calls.

**New batch model: `scribe_v2_medical` (September 11, 2026 changelog):** Fine-tune of Scribe v2 specialized for medical/clinical audio — improved recognition of drug names, anatomy, pathology, and clinical dictation. `model_id="scribe_v2_medical"` on batch Create transcript endpoint. Same features as `scribe_v2`: keyterm prompting (up to 1,000), entity detection, speaker diarization, no-verbatim mode. Same pricing as Scribe v2 ($0.22/hr base; $0.27/hr with keyterms). HIPAA-eligible with BAA for Enterprise; Zero Retention Mode available (audio + text deleted immediately after each request). **Zero impact on Snelverhuizen pipeline** — Dutch moving-company VO QA has no medical content; continue using `model_id="scribe_v2"` for all VO QA and caption alignment calls.
...
**SDK v2.65.0 (August 25, 2026) — current latest.** History from v2.59.0:
- **v2.59.0 (July 22):** HMAC-signed post-call webhook config for on-prem conversations — no audio pipeline changes.
- **v2.60.0 (July 30):** AsyncConversation startup fix; list-of-primitive multipart field handling fix — no audio API changes.
- **v2.61.0 (August 5):** Stale patch fix + SDK regen — no audio API changes.
- **v2.62.0 (August 6):** SDK regen only — no audio API changes.
- **v2.63.0 (August 11):** Added missing **realtime STT** options. New realtime model IDs: `scribe_v2_realtime_turbo` and `scribe_v2_realtime_lite`. New realtime WebSocket params: `secondary_languages` (list of additional language codes), `filter_background_audio` (bool — filters background noise before transcription), `enable_logging`, single-use token auth, entity detection, final transcript events. **Batch Scribe v2 API is UNCHANGED** — all existing `client.speech_to_text.convert()` calls in §11 are unaffected. Realtime additions are WebSocket-only.
- **v2.64.0 (August 14):** `extra_setup_config` parameter for on-premises deployment messaging — no audio pipeline changes.
- **v2.65.0 (August 25):** SDK regeneration. Fixes repeated-form-field encoding for keyterms/webhook_ids (Conversational AI platform only). **Zero TTS/SFX/audio-tag pipeline impact** — all `text_to_speech.convert()`, `text_to_sound_effects.convert()`, and `speech_to_text.convert()` calls in this skill file are unaffected.

All TTS, SFX v2, VoiceSettings, AllowedOutputFormats, and batch Scribe v2 API calls in this skill file are confirmed valid on v2.65.0.
...
**SC302 recheck (Aug 28, 2026):** v2.65.0 released Aug 25 — SDK regen fixing multipart field encoding for keyterms/webhook_ids (Agents Platform only; zero TTS/SFX impact — see v2.65.0 entry above). yt-dlp 2026.08.19 still current stable (confirmed — no new stable since Aug 19). ffmpeg-normalize v1.41.1 still current (confirmed — no new release since July 10). ElevenLabs eleven_v3 audio tags: no new confirmed tags since prior rechecks. NoorLoops music-library tier labels corrected from Conservative/Moderate/Broad → Voice-only/Ambience-only / Percussion-light / Broad-use (SC302 finding — confirmed via NoorLoops search; SFX library retains Conservative/Moderate/Broad).

**SC309 recheck (Aug 30, 2026):** ElevenLabs SDK v2.65.0 still latest (confirmed — no new release since Aug 25). yt-dlp 2026.08.19 still current stable (confirmed — no new stable release since Aug 19; nightly builds through Aug 29 exist but are not stable channel). ffmpeg-normalize v1.41.1 still current (confirmed PyPI — no new release since July 10). Scribe v2 batch API: August changelog entries (Aug 3 + Aug 10) are realtime WebSocket-only changes (`entity_detection` in realtime, new realtime model IDs `scribe_v2_realtime_turbo`/`scribe_v2_realtime_lite`, `secondary_languages`, `filter_background_audio`) — all realtime-only; batch `client.speech_to_text.convert()` calls in §11 are unchanged. ElevenLabs audio tags: no new confirmed tags. PRIMARY ADDITION (SC309): Lomeyo Nasheed Directory added as new source in §1 — first machine-queryable halal catalog with instrumentation metadata designed for AI agents; public API at `/api/public/` (no key); CANARY required before production. CORRECTION (SC309): Aswati track count updated 70+ → 90+ (confirmed Aug 2026 search results).

**SC313 recheck (Sept 1, 2026):** ElevenLabs SDK v2.65.0 still latest — confirmed via PyPI and GitHub releases; no v2.66+ released as of Sept 1. yt-dlp 2026.08.19 still current stable — confirmed via GitHub releases page; no new stable release in September (nightly builds exist separately). ffmpeg-normalize v1.41.1 still latest — no new release since July 10. ElevenLabs audio tags: no new confirmed tags found in official ElevenLabs sources. Lomeyo Nasheed Directory: no new updates since June 30, 2026 — CANARY status unchanged (still required before production use). CORRECTION (SC313): `apply_text_normalization="on"` on Flash v2.5 wording upgraded from "may be Enterprise-only" → "confirmed Enterprise-only" (multi-source confirmation: ElevenLabs docs + third-party API references consistent; Known Issues §8 entry unchanged). All TTS, SFX v2, VoiceSettings, AllowedOutputFormats, and batch Scribe v2 API calls in this skill file remain valid on SDK v2.65.0.

**SC320 recheck (Sept 2, 2026):** ElevenLabs SDK **v2.66.0** released Sept 2. Change: `file_ids` parameter added to `send_multimodal_message` (Conversational AI Agents Platform multimodal messaging only — new contributor @bsreenivas11). **Zero TTS/SFX/Scribe/audio-tag pipeline impact.** All `text_to_speech.convert()`, `text_to_sound_effects.convert()`, and `speech_to_text.convert()` batch calls in this skill file remain valid on v2.66.0. yt-dlp 2026.08.19 still current stable (confirmed via GitHub API — no new stable release since Aug 19). **ffmpeg-normalize v1.42.0** released Sept 2 — new `--write-encoder-settings` option; no loudness-target or command-syntax changes (see §4f version history). All existing ffmpeg-normalize commands unchanged. ElevenLabs changelog proxy-blocked; confirmation from SDK release notes only.

**SC327 recheck (Sept 5, 2026):** ElevenLabs SDK v2.66.0 still latest (confirmed PyPI + GitHub releases — no new release since Sept 2). **Sept 3 post-release commit on main:** `fix: strip the OMIT sentinel before it reaches the realtime TTS connection` (#858 — authors: Kayvan-Zahiri + claude). This is a realtime WebSocket TTS fix — **zero impact on batch `text_to_speech.convert()`, `text_to_sound_effects.convert()`, or `speech_to_text.convert()` calls**. Not yet released as a new SDK version (no v2.67.0 cut as of Sept 5). yt-dlp 2026.08.19 still current stable (confirmed GitHub releases — no September stable release yet; monthly cadence suggests one may come late Sept). ffmpeg-normalize v1.42.0 still latest (confirmed GitHub releases — no release since Sept 2). ElevenLabs audio tags: no new confirmed tags from official sources. Halal audio sources: no new providers or catalog updates found (Aswati 90+ stable, NoorLoops unchanged, Lomeyo CANARY status unchanged). All TTS, SFX v2, VoiceSettings, AllowedOutputFormats, and batch Scribe v2 API calls in this skill file remain valid on SDK v2.66.0.

**SC334 recheck (Sept 7, 2026):** ElevenLabs SDK v2.66.0 still latest (confirmed GitHub releases — no v2.67.0 cut as of Sept 7). yt-dlp 2026.08.19 still current stable (confirmed GitHub releases — nightly builds through Aug 30 exist but no new stable release; monthly cadence). ffmpeg-normalize v1.42.0 still latest (confirmed GitHub releases — no release since Sept 2). **ElevenLabs Aug 31 changelog (previously unreviewed):** All Agents Platform changes (DTMF keypad input, conversation attachment history, agent tag filtering, batch campaign context) + Music: `with_waveform_visual` parameter added to music endpoints (`generate_music_detailed`, `stream_music`, `upload_music`). **Zero TTS/SFX v2/Scribe batch pipeline impact.** The music update confirms `music_v2` is actively receiving API changes — §2 Music API DO NOT USE warning and `model_id="music_v2"` documentation updated accordingly. **No September 2026 ElevenLabs changelog entries exist yet as of Sept 7** (latest official entry is Aug 31). ElevenLabs audio tags: no new confirmed tags from official sources. Halal audio sources: no new providers or catalog changes (Aswati 90+ stable, NoorLoops unchanged, Lomeyo CANARY status unchanged). All TTS, SFX v2, VoiceSettings, AllowedOutputFormats, and batch Scribe v2 API calls in this skill file remain valid on SDK v2.66.0.

**SC341 recheck (Sept 8, 2026):** ElevenLabs SDK **v2.67.0** released Sept 7 (SC334 missed it — likely released after that session). Two changes: (1) fix: OMIT sentinel no longer leaks into the realtime TTS WebSocket connection (realtime-only fix; contributor Kayvan-Zahiri); (2) SDK regeneration. **Zero impact on batch `text_to_speech.convert()`, `text_to_sound_effects.convert()`, or `speech_to_text.convert()` calls** — this pipeline exclusively uses batch/REST, not realtime WebSocket TTS. SC327 had tracked the underlying commit (Sept 3) but it was not yet versioned; v2.67.0 is now the correct current version. Additionally, **v3.0.0-alpha.1** pre-release dropped Sept 8 (today): major SDK v3 regeneration — agent renaming, orchestrator config, file_ids on multimodal messaging (Agents Platform only). **DO NOT upgrade to v3.0.0-alpha.1 in production** — it is a breaking pre-release; all `elevenlabs~=2.67` batch API calls in this pipeline remain valid and unaffected. yt-dlp 2026.08.19 still current stable (confirmed — no new September stable release). ffmpeg-normalize v1.42.0 still latest (confirmed — no release since Sept 2). ElevenLabs September 2026 API changelog: proxy-blocked; based on SDK v2.67.0 release notes, only realtime TTS fix — no TTS/SFX v2/Scribe batch changes. All TTS, SFX v2, VoiceSettings, AllowedOutputFormats, and batch Scribe v2 API calls in this skill file remain valid on SDK v2.67.0.

**SC348 recheck (Sept 10, 2026):** ElevenLabs SDK v2.67.0 still latest stable (confirmed GitHub releases — no new stable release between Sept 8–10; v3.0.0-alpha.1 remains the only Sept 8 drop and is still pre-release). yt-dlp 2026.08.19 still current stable (confirmed GitHub releases page — no new stable release; nightly builds exist but no stable channel update). ffmpeg-normalize v1.42.0 still latest (confirmed — no release since Sept 2). ElevenLabs local MCP server deprecated in favour of hosted MCP at `https://api.elevenlabs.io/v1/mcp` (OAuth sign-in required) — **zero pipeline impact** (pipeline uses Python SDK / REST API directly, not MCP). New Scribe realtime model IDs `scribe_v2_realtime_turbo` and `scribe_v2_realtime_lite` — already documented in §11 and §12 (v2.63.0 Aug 11 entry); realtime-WebSocket-only; batch `scribe_v2` unchanged. Halal audio sources: no new providers, no catalog changes (Aswati 90+ stable, NoorLoops unchanged, Lomeyo CANARY status unchanged). All TTS, SFX v2, VoiceSettings, AllowedOutputFormats, and batch Scribe v2 API calls in this skill file remain valid on SDK v2.67.0.

**SC355 recheck (Sept 12, 2026):** ElevenLabs SDK **v2.68.0** released September 11 — `compose_detailed` now accepts every music model ID and composition plan type (contributor @hrist0stoichev) plus SDK regeneration. **Zero TTS/SFX v2/Scribe batch pipeline impact** — change is music API only. All `text_to_speech.convert()`, `text_to_sound_effects.convert()`, and `speech_to_text.convert()` batch calls in this skill file remain valid on v2.68.0. v3.0.0-alpha.1 still the only pre-release (Sept 8); do NOT upgrade to it in production. yt-dlp 2026.08.19 still current stable (confirmed — no new September stable release). ffmpeg-normalize v1.42.0 still latest (confirmed — no release since Sept 2). ElevenLabs Sept 11 changelog: proxy-blocked; SDK release confirms only music compose_detailed changes — no TTS/SFX v2/Scribe batch API changes. Halal audio sources: no new providers or catalog changes (Aswati 90+ stable, NoorLoops unchanged, Lomeyo CANARY status unchanged). All TTS, SFX v2, VoiceSettings, AllowedOutputFormats, and batch Scribe v2 API calls in this skill file remain valid on SDK v2.68.0.

**SC362 recheck (Sept 14, 2026):** ElevenLabs SDK v2.68.0 still latest stable (confirmed GitHub releases — no v2.69.0 as of Sept 14; no unreleased commits on main since Sept 11). yt-dlp 2026.08.19 still current stable (confirmed GitHub releases — no new stable release). ffmpeg-normalize v1.42.0 still latest (confirmed PyPI — no release since Sept 2). **SC355 miss corrected (Sept 14):** The September 11, 2026 ElevenLabs changelog also introduced `scribe_v2_medical` — a new batch Scribe v2 fine-tune for medical/clinical audio. SC355 only captured the SDK v2.68.0 music API change (compose_detailed); this API-level model addition was missed. `scribe_v2_medical` is now documented in §11 above. **Zero Snelverhuizen pipeline impact** — use `scribe_v2` for Dutch VO QA. Halal audio sources: no new providers or catalog changes (Aswati 90+ stable, NoorLoops unchanged, Lomeyo CANARY status unchanged). All TTS, SFX v2, VoiceSettings, AllowedOutputFormats, and batch Scribe v2 API calls in this skill file remain valid on SDK v2.68.0.
