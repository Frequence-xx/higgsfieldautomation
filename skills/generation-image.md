...
**✅ 2026-08-08: Grok Imagine Image 2.0 developer API opened.** xAI replaced the Quality model with Image 2.0 on August 7, 2026 (consumer) and opened developer API access on **August 8**. Model ID on xAI native: `grok-imagine-image-2.0`; on OpenRouter: `x-ai/grok-imagine-image-2.0` ($0.04/img at low/1K, $0.08/img at medium/2K; also $0.01/img image input). Also on fal.ai and Replicate. Key upgrade: region-specific editing (magic wand, segmentation, background removal), auto-resize across 9 aspect ratios, designer-grade typography, arena **#2 for both T2I and image-editing** (behind GPT Image 2). **⚠️ CRITICAL ref count correction (2026-08-20 pass 41):** ref ceiling is **3 via API** — the 5-ref figure applies to consumer apps only (grok.com, iOS, Android). API spec (OpenRouter / fal.ai / Replicate) consistently states max 3 input images. **AIMLAPI status: NOT on AIMLAPI as of 2026-08-20 (pass 41 recheck)** — AIMLAPI docs/product search returns only the old `x-ai/grok-imagine-image` (March 2026 basic model); no `grok-imagine-image-2.0` page found. Expected AIMLAPI model string when added: `x-ai/grok-imagine-image-2.0`. At 3-ref ceiling and $0.04/img (1K), this is the cheapest B-roll/environment draft once AIMLAPI adds it — but still NOT suitable for character shots (no face adherence, 3-ref ceiling insufficient for char sheet + truck + boxes). Do NOT use before canary confirms AIMLAPI availability.
...
⊘**Qwen-Image-3.0 (launched 2026-07-21, GA 2026-08-05 — NO API ON AIMLAPI as of 2026-08-15):** Alibaba's third-generation image model. Available in two editions: **Qwen-Image-3.0-Pro** (native Alibaba API model ID: `qwen-image-3.0-pro`) and **Qwen-Image-3.0-Standard** (model ID: `qwen-image-3.0-standard`). Both went fully open on Alibaba Cloud Bailian/DashScope on August 5, 2026 (from invite-only preview since July 21). Key specs: up to 4.5K token prompt input; sub-10px text legibility in 12 languages (Dutch confirmed); complex multi-panel layout generation (newspapers, storyboards, menus, exam papers in one pass); image editing that preserves character appearance across scenes; 9 images at once in one call; micro-expression and pore-level fidelity in portraits. **Pipeline status (2026-08-20 pass 41 recheck): AIMLAPI model database page (docs.aimlapi.com/api-references/model-database) appears to list `alibaba/qwen-image-3` and `alibaba/qwen-image-3-edit` — no dedicated docs pages found yet (same situation as FLUX.2 Max Edit at product-page-only stage). If confirmed, expected model strings: `alibaba/qwen-image-3` (Standard) and `alibaba/qwen-image-3-edit` (Pro edit). CANARY REQUIRED before any production use — parameter names, pricing, 9:16 support, and max ref count all unverified. Primary use case if confirmed: CTA cards requiring Dutch text at sub-10px legibility, and character compositing with 4.5K token multi-ref prompts. **Monitor for dedicated docs page addition on docs.aimlapi.com.**
