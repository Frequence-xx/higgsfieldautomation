...
| FLUX.2 Max Edit | `blackforestlabs/flux-2-max-edit`⊕ | Highest-quality I2I editing; upgrade from Pro Edit for money-shot inpainting; 3 refs — CANARY REQUIRED | 3 | ~$0.10 | native |
...
| MAI-Image 2.5 | `microsoft/mai-image-2.5`⟁ (**CONFIRMED on AIMLAPI 2026-07-01 — CANARY REQUIRED**) | T2I + I2I editing; #2 Arena image-edit, #3 T2I; face identity preservation across pose/expression; strong text on labels/packaging | TBD | token-based (~$0.10–0.20 est) | 9:16 native (`"aspect_ratio": "9:16"`) |
...
⊕**FLUX.2 Max Edit (2026-06-22 — NEW, CANARY REQUIRED):** BFL's highest-quality I2I editing model. **AIMLAPI product page confirmed at aimlapi.com/models/flux-2-max-edit** — model is listed and available. **⚠️ 2026-07-01 update: dedicated docs page at docs.aimlapi.com NOT yet published** — only the product landing page exists; parameter syntax unverified. Expected AIMLAPI model string: `blackforestlabs/flux-2-max-edit` (follows FLUX.2 naming pattern). Supports up to 3 reference images (same slot count as FLUX.2 Pro Edit). **Key advantage over FLUX.2 Pro Edit:** better photorealistic fidelity, more precise color/lighting/face/text preservation, less visual drift. Use case: money-shot inpainting and brand-critical I2I edits where FLUX.2 Pro Edit quality is insufficient. Pricing: ~$0.10/img estimated (canary to confirm). Parameters expected to mirror FLUX.2 Pro Edit: `prompt`, `image_urls` (1-3), `image_size`, `output_format`, `safety_tolerance` (1-5, default 2). **CANARY REQUIRED** before production use — docs page not published, parameter syntax unverified on AIMLAPI proxy.
