...
- **AIMLAPI proxy status (unconfirmed):** AIMLAPI docs page is blocked — these params are confirmed via fal.ai and BFL native; AIMLAPI may expose them under the same names or may silently ignore them. Run a canary before adding to production calls. If AIMLAPI doesn't expose them, these settings are not actionable via our pipeline.
