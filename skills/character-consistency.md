...
**When to test Wan 2.7 R2V on AIMLAPI (pass 28 update, 2026-07-09):** Model ID `alibaba/wan-2-7-r2v` is now appearing in the AIMLAPI model database. Next production session: attempt one canary draft — Karel/Mourad front.png as `reference_images[0]`, 720p, `generate_audio: false`. Score with InsightFace buffalo_l (PASS threshold 0.62). If the call succeeds and identity score ≥ 0.62 across 3 runs, it's eligible for draft-tier use at ~$0.625/5s (vs $1.09/5s for Kling v3 Standard) — significant savings for iteration loops. Only adopt for production finals after owner-reviewed output passes brand binary checklist. If the canary returns a model-not-found error, fall back to Wan 2.6 R2V or Kling O1.
