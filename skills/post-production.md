...
Note on `@remotion/install-whisper-cpp`: Remotion's install package recommends v1.5.5 and has no Windows binary for v1.9.x. Our pipeline uses whisper.cpp CLI on Linux server (cmake build) — unaffected by Remotion's binary limitation. Continue using whisper.cpp v1.9.3 CLI directly per `captions-and-titles.md`.
