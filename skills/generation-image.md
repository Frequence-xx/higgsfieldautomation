...
| MAI-Image 2.5 | `microsoft/mai-image-2.5`⟁ (**NOT confirmed on AIMLAPI — monitor**) | T2I + I2I editing; #2 Arena image-edit, #3 T2I; face identity preservation across pose/expression; strong text on labels/packaging | TBD | token-based (~$0.10–0.20 est) | 9:16 supported |
...
⟁**MAI-Image 2.5 (Microsoft, launched 2026-06-02 — NOT confirmed on AIMLAPI, monitor monthly):** Microsoft's strongest image model. T2I + I2I editing in one model. Ranked **#2 on Arena image-editing** leaderboard (ahead of NB2) and **#3 for T2I** (surpassing GPT-Image-1.5 and NBP 2K). Two variants: MAI-Image-2.5 (full fidelity) and MAI-Image-2.5-Flash (faster, ~4× cheaper). Key capabilities: face identity preservation across pose, expression, and viewpoint changes; strong in-image text rendering (labels, packaging, CTA cards); commercial product photography with accurate lighting/scale/spatial structure. **Pricing (token-based, like GPT Image 2):** Full: $5/M text input + $8/M image input + $47/M image output tokens (~$0.10–0.20/img estimated at 1K). Flash: $1.75/M all token types (~$0.03–0.07/img estimated). **Availability:** Azure AI Foundry (primary), OpenRouter, fal.ai (`microsoft/mai-image-2.5`), MAI Playground. **AIMLAPI status: unconfirmed** — one AIMLAPI search index mentions it under `microsoft/mai-image-2.5`, but this is not directly verified from the AIMLAPI docs page. Under AIMLAPI-only policy, do NOT use it until AIMLAPI listing is confirmed via canary. **If confirmed on AIMLAPI**, use case: (a) face-consistent character editing chain at cost below NBP ($0.195); (b) CTA cards with Dutch text (Arena #2 editing score suggests strong instruction following); (c) Flash variant as cheap draft tier below Gemini 2.5 Flash Image. CANARY REQUIRED before any production use.
