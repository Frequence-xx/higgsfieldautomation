...
- [ ] Remotion version recheck (SC316, 2026-09-01): latest is **v4.0.520** (released Sept 1, 2026). Changes: Studio UI only — `playbackStore` / `usePlaying()` hook replaces imperative playing ref; waveform fidelity tied to composition FPS; no new production effects. New package `@remotion/whisper-webgpu` added in v4.0.518 (Aug 26) — **browser-WASM transcription only, NOT relevant to our server-side pipeline**; our WhisperX + whisper.cpp CLI word-level timestamp path (captions-and-titles.md) is unchanged and correct. New `@remotion/gsap` (v4.0.517) adds GSAP animation support — not needed for our pipeline. Social safe zones overlay (v4.0.518) is a Studio preview tool only. No action required; upgrade to v4.0.520 when next touching Remotion compositions.
