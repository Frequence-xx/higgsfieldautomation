...
**O3 not on AIMLAPI as of 2026-07-23 (pass 35 recheck).** docs.aimlapi.com search confirms only v3-standard, v3-standard-turbo, v2.6-motion-control, v2-master — no O3 endpoint found. O3 confirmed on Runware, fal.ai, Atlas Cloud. Per Farouq directive — AIMLAPI-only pipeline. Continue using Kling O1 until O3 lands on AIMLAPI.
...
**Wan 2.7 R2V status on AIMLAPI (pass 35 recheck, 2026-07-23):** docs.aimlapi.com still has no dedicated Wan 2.7 R2V page — only `wan-2.7-image-to-video` (I2V) and `wan-2.6-reference-to-video` confirmed. However, AIMLAPI published a blog post covering Wan 2.7 T2V/I2V/R2V/Edit as "next-generation" modes — suggesting R2V support may now be live. Status upgraded from "not live" to **"canary-test recommended"**: call `alibaba/wan-2-7-r2v` with Karel `front.png` at 720p, muted audio, score with InsightFace (PASS ≥ 0.62). If the endpoint returns `model-not-found`, fall back to `wan-2.6-reference-to-video` or Kling O1. Do NOT promote to production without owner-reviewed output passing brand binary checklist. All third-party providers (Segmind, Replicate, Together AI, Kie.ai, EvoLink, inference.sh, WaveSpeedAI) have Wan 2.7 R2V confirmed live.
