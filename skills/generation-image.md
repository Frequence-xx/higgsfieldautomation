...
|-------|---------------|----------|----------|------|-------------|
| Nano Banana 2 Lite | `google/gemini-3.1-flash-lite-image`★ (**CONFIRMED ON AIMLAPI 2026-07-03 — CANARY REQUIRED**) | Ultra-cheap T2I + I2I + multi-image composition; ~4s/image; replaces Gemini 2.5 Flash Image. Cheaper than NB2 ($0.034 native vs $0.067). Use for layout/composition drafts before NB2 pass. NOT for final hero frames. | TBD (I2I confirmed) | ~$0.034 (native; AIMLAPI rate: canary) | 9:16 native |
...
★**Nano Banana 2 Lite (Gemini 3.1 Flash Lite Image, released 2026-06-30 — CONFIRMED ON AIMLAPI 2026-07-03):** Google's cheapest and fastest image model. AIMLAPI product page confirmed at `aimlapi.com/models/nano-banana-2-lite`. Expected AIMLAPI model string: **`google/gemini-3.1-flash-lite-image`** (follows AIMLAPI's native-string convention for Gemini models; alternative alias `google/nano-banana-2-lite` also possible — CANARY REQUIRED to confirm). Key specs: ~4 seconds/image; $0.034/img at 1K (native Google; AIMLAPI rate TBD — run canary). Supports T2I + image editing + multi-image composition in one model — "a single drop-in API handles text-to-image, image editing, and multi-image composition" (Google blog, 2026-06-30). Max reference images: TBD (CANARY REQUIRED — declared as multimodal with image input support). **Designed as the direct replacement for Gemini 2.5 Flash Image** (`google/gemini-2.5-flash-image`, retiring Oct 2, 2026) — use NB2 Lite as the cheapest draft tier going forward. Quality: "reliable prompt adherence, strong character consistency, legible in-image text" (VentureBeat). NOT suitable for final hero frames or character identity shots — use NB2 for those. If NB2 Lite refs are confirmed in canary, it could displace NB2 as the primary iteration model at ~50% lower cost.
...
```
Need ultra-cheap T2I layout/composition draft (no chars, no refs)? → NB2 Lite ($0.034, `google/gemini-3.1-flash-lite-image`) — also supports I2I — CANARY REQUIRED; fallback: Gemini 2.5 Flash Image ($0.039, retiring Oct 2, 2026)
