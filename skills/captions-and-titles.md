...
1. **Extract word-level timestamps** — three options in priority order:
   ...
   **Mode A (DTW, recommended):** `tokenLevelTimestamps: true` adds `--dtw` to whisper.cpp. Produces the most accurate single-point timestamp per word (`t_dtw`). `tokensPerItem` and `splitOnWord` have **no effect** in DTW mode.
   ...
   const WHISPER_VERSION = '1.9.4';        // v1.9.4 is current latest STABLE (Sep 11, 2026) — GGML v0.23.0 sync, decoder reseeding fix, language-detect improvement; no DTW/timestamp changes. v1.9.3 stable (Aug 20, 2026). v1.9.2 — VAD token timestamp fix (PR #3910). (SC354 Sep 12, 2026)
   ...
   const result = await transcribe({
     ...
     whisperCppVersion: WHISPER_VERSION, // REQUIRED — must match install version; use '1.9.4' (current stable, Sep 11, 2026)
   ...
   **Model caching pattern (important for cloud/containerized pipeline):**
   ...
   const WHISPER_VERSION = '1.9.4';        // current stable (Sep 11, 2026)
...
- **Studio Elements: "basic captions element" added** — a drag-and-drop pre-built caption component installable from Studio's element library (Studio UI only; not a change to `@remotion/captions` API or production pipeline code).
- `@remotion/whisper-webgpu`: Options and hosted models added (not relevant — Option D WASM-only, no large-v3-turbo, not for Dutch production).
- **No changes to `@remotion/captions` API or `@remotion/install-whisper-cpp`.** Caption pipeline unaffected. SC347 Sep 10, 2026 recheck.

**Remotion v4.0.522 (September 7, 2026):**
- `@remotion/cli`: Show command options in `--help`.
- `@remotion/studio`: Timeline and UI refinements (audio asset zoom, timeline layer resize clamping, Studio link imports).
- **No changes to `@remotion/captions` or `@remotion/install-whisper-cpp`.** Caption pipeline unaffected. SC339 Sep 8, 2026 recheck.

**Remotion v4.0.521 (September 5, 2026):**
...
**Remotion v4.0.518 (August 26, 2026):**
...
FFmpeg 8.0 "Huffman" (August 2025) added a native `whisper` audio filter that performs ASR transcription inside FFmpeg itself using whisper.cpp. Current stable: **FFmpeg 9.0.1 "Lei" (August 12, 2026)** — confirmed by SC305 Aug 29, 2026. FFmpeg 9.0 has no changes to the drawtext or whisper filters (SC308 Aug 30, 2026). **Do NOT use this for Snel Verhuizen caption pipeline.** Reason: it outputs segment-level SRT/JSON only — no word-level timestamps, no word-by-word highlighting. Suitable only for basic subtitle overlays without per-word orange highlight. Our pipeline requires word-level timestamps from Options A/B/C above.
