...
| Wan 2.7 R2V | `alibaba/wan-2-7-r2v` | 720p/1080p | ~$0.50 est. | **LIKELY LIVE ON AIMLAPI (2026-07-11)** — AIMLAPI blog post confirms "all four modes (T2V, I2V, R2V, and video editing) via the AI/ML API platform." Docs navigation entry appears. Model string `alibaba/wan-2-7-r2v` present in model database. `image_urls` max 3 image-only refs (total refs including videos ≤ 5). Duration cap 2-10s for R2V. **CANARY REQUIRED before production.** Wan 2.6 R2V (`alibaba/wan-2-6-r2v`) remains fallback if R2V canary fails. |
