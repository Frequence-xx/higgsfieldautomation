...
| tail_image_url | string | — | End frame for transitions. **Native Kling API name: `image_tail`; AIMLAPI wrapper: `tail_image_url` (probable — one AIMLAPI-context search snippet confirmed this name July 2026; full canary still required).** **⚠️ Pro-only in native Kling API** (SC223 July 18, 2026: Node.js wrapper documents `image_tail` as "end frame for interpolation (pro mode)") — Standard/Standard Turbo may not support end-frame conditioning natively. AIMLAPI wrapper behavior may differ — canary with Standard Turbo to confirm. **Incompatible with multi_prompt.** **Incompatible with static_mask_url, dynamic_masks, and camera_control (Kling v3 mutual exclusivity — pick only ONE of these four per call).** Same image = forces stationarity but allows mid-clip drift. |
