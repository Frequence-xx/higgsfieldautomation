...
| Grok Imagine Image 2.0 | NOT ON AIMLAPI YET† (**API open on xAI/OpenRouter/fal/Replicate since 2026-08-08 — NOT on AIMLAPI as of 2026-08-29 (pass 45 recheck)**) | #2 Arena T2I + editing; up to 3 refs via API (5 in consumer apps only); region editing, designer typography; 9 aspect ratios via Smart Resize; replaces Grok Imagine Quality model | 3 (API) | $0.04/img (1K) via OpenRouter | 9:16 supported |
...
**✅ 2026-08-08: Grok Imagine Image 2.0 developer API opened.** xAI replaced the Quality model with Image 2.0 on August 7, 2026 (consumer) and opened developer API access on **August 8**. Model ID on xAI native: `grok-imagine-image-2.0`; on OpenRouter: `x-ai/grok-imagine-image-2.0` ($0.04/img at low/1K, $0.08/img at medium/2K; also $0.01/img image input). Also on fal.ai and Replicate. Key upgrade: region-specific editing (magic wand, segmentation, background removal), auto-resize across 9 aspect ratios, designer-grade typography, arena **#2 for both T2I and image-editing** (behind GPT Image 2). **⚠️ CRITICAL ref count correction (2026-08-20 pass 41):** ref ceiling is **3 via API** — the 5-ref figure applies to consumer apps only (grok.com, iOS, Android). API spec (OpenRouter / fal.ai / Replicate) consistently states max 3 input images. **AIMLAPI status: NOT on AIMLAPI as of 2026-09-06 (pass 48 recheck)** — AIMLAPI docs/product search returns only the old `x-ai/grok-imagine-image` (March 2026 basic model) and the deprecated `x-ai/grok-imagine-image-pro`; no `grok-imagine-image-2.0` page found. Native model string confirmed as `grok-imagine-image-2.0` (xAI native). Expected AIMLAPI model string when added: `x-ai/grok-imagine-image-2.0`. At 3-ref ceiling and $0.04/img (1K), this is the cheapest B-roll/environment draft once AIMLAPI adds it — but still NOT suitable for character shots (no face adherence, 3-ref ceiling insufficient for char sheet + truck + boxes). Do NOT use before canary confirms AIMLAPI availability.
